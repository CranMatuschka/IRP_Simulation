...
assert(isequaln( ...
    ...
    history.errors.atmosphere.residual_m));
assert(isequaln( ...
    reportData.atmosphere_troposphere_residual_by_receiver_tower_m, ...
    history.errors.troposphere.residual_m));
assert(isequaln( ...
    reportData.atmosphere_ionosphere_residual_by_receiver_tower_m, ...
    history.errors.ionosphere.residual_m));
assert(abs(reportData.model_atmosphere_residual_variance_m2 - ...
    mean(history.errors.atmosphere.variance_m2(:), 'omitnan')) < eps);

results = ResultBuilder.fromSimulation(sim);
assert(isequaln( ...
    results.atmosphere_truth_troposphere_by_receiver_tower_m, ...
    history.errors.troposphere.deterministicTruth_m));
assert(isequaln( ...
    results.atmosphere.residual.total_m, ...
    history.errors.atmosphere.residual_m));
assert(isequaln( ...
    results.non_atmospheric.truth.hardware_m, ...
    history.errors.hardware.truth_m));
