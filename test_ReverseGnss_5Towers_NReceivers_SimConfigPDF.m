...
function runStochasticTruthResidualReportDiagnosticsRegression(sim, reportText)
    mainReportData = ReportDataBuilder.fromSimulation(sim);

    assert(isfield(mainReportData, ...
        'stochastic_truth_residual_summary_table'), ...
        'Main report data should contain stochastic truth residual summary table.');

    assert(isfield(mainReportData, ...
        'truth_atmosphere_residual_rms_m'), ...
        'Report data should export total truth atmosphere residual RMS.');

    assert(isfield(mainReportData, ...
        'truth_troposphere_residual_rms_m'), ...
        'Report data should export truth troposphere residual RMS.');

    assert(isfield(mainReportData, ...
        'truth_ionosphere_residual_rms_m'), ...
        'Report data should export truth ionosphere residual RMS.');

    assert(any(contains( ...
        string(mainReportData.stochastic_truth_residual_summary_table.Quantity), ...
        "Sample RMS total truth residual")), ...
        'Stochastic truth residual summary table should include total residual RMS.');

    assert(any(contains( ...
        string(mainReportData.stochastic_truth_residual_summary_table.Quantity), ...
        "Configured truth total residual sigma")), ...
        'Stochastic truth residual summary table should include configured total sigma.');
    ...
    stochasticReportData = ReportDataBuilder.fromSimulation(stochasticSim);

    assert(stochasticReportData.truth_troposphere_residual_config_sigma_m > 0.0, ...
        'Stochastic report data should record nonzero truth troposphere residual sigma.');

    assert(stochasticReportData.truth_ionosphere_residual_config_sigma_m > 0.0, ...
        'Stochastic report data should record nonzero truth ionosphere residual sigma.');

    assert(stochasticReportData.truth_total_residual_config_sigma_m > ...
            max(stochasticReportData.truth_troposphere_residual_config_sigma_m, ...
                stochasticReportData.truth_ionosphere_residual_config_sigma_m), ...
        'Configured total residual sigma should be RSS of troposphere and ionosphere sigmas.');

    assert(stochasticReportData.truth_atmosphere_residual_rms_m > 0.0, ...
        'Stochastic report data should record nonzero total truth residual RMS.');

    assert(stochasticReportData.truth_troposphere_residual_rms_m > 0.0, ...
        'Stochastic report data should record nonzero troposphere truth residual RMS.');

    assert(stochasticReportData.truth_ionosphere_residual_rms_m > 0.0, ...
        'Stochastic report data should record nonzero ionosphere truth residual RMS.');

    assert(abs(stochasticReportData.truth_total_residual_config_sigma_m - ...
        hypot(stochasticReportData.truth_troposphere_residual_config_sigma_m, ...
              stochasticReportData.truth_ionosphere_residual_config_sigma_m)) < 1e-12, ...
        'Configured total residual sigma should equal hypot(troposphere sigma, ionosphere sigma).');

    assert(abs(stochasticReportData.truth_atmosphere_residual_rms_m - ...
        sqrt(mean( ...
        stochasticReportData.atmosphere_truth_residual_by_tower_m(:).^2, ...
        'omitnan'))) < 1e-12, ...
        'Report total residual RMS should match raw truth residual samples.');

    assert(abs(stochasticReportData.truth_troposphere_residual_rms_m - ...
        sqrt(mean( ...
        stochasticReportData.atmosphere_truth_troposphere_residual_by_tower_m(:).^2, ...
        'omitnan'))) < 1e-12, ...
        'Report troposphere residual RMS should match raw truth residual samples.');

    assert(abs(stochasticReportData.truth_ionosphere_residual_rms_m - ...
        sqrt(mean( ...
        stochasticReportData.atmosphere_truth_ionosphere_residual_by_tower_m(:).^2, ...
        'omitnan'))) < 1e-12, ...
        'Report ionosphere residual RMS should match raw truth residual samples.');

    assert(any(contains( ...
        string(stochasticReportData.stochastic_truth_residual_summary_table.Quantity), ...
        "Sample std total truth residual")), ...
        'Stochastic truth residual summary table should include sample standard deviation.');
...
function runTruthResidualStatisticsRegression()
    ...
    assert(abs(reportData.truth_troposphere_residual_config_sigma_m - ...
        expectedTroposphereSigma_m) < 1e-12, ...
        'Report data should retain configured truth troposphere residual sigma.');

    assert(abs(reportData.truth_ionosphere_residual_config_sigma_m - ...
        expectedIonosphereSigma_m) < 1e-12, ...
        'Report data should retain configured truth ionosphere residual sigma.');

    assert(abs(reportData.truth_total_residual_config_sigma_m - ...
        expectedTotalSigma_m) < 1e-12, ...
        'Report data should compute configured total residual sigma as RSS.');

    assert(abs(reportData.truth_troposphere_residual_std_m - ...
        troposphereStd_m) < 1e-12, ...
        'Report troposphere residual std should match raw samples.');

    assert(abs(reportData.truth_ionosphere_residual_std_m - ...
        ionosphereStd_m) < 1e-12, ...
        'Report ionosphere residual std should match raw samples.');

    assert(abs(reportData.truth_atmosphere_residual_std_m - ...
        totalStd_m) < 1e-12, ...
        'Report total residual std should match raw samples.');
...
function runInnovationCovarianceReportDiagnosticsRegression(sim, reportText)
    ...
    assert(isfield(reportData, ...
        ...
        'Report data should export mean innovation covariance sigma.');

    assert(isfield(reportData, ...
        'innovation_covariance_summary_table'), ...
        'Report data should contain innovation covariance summary table.');

    assert(any(contains( ...
        string(reportData.innovation_covariance_summary_table.Quantity), ...
        "Mean NIS per degree of freedom")), ...
        'Innovation covariance summary table should include mean NIS per degree of freedom.');

    assert(any(contains( ...
        string(reportData.innovation_covariance_summary_table.Quantity), ...
        "Mean atmosphere R contribution sigma")), ...
        'Innovation covariance summary table should include atmosphere R contribution sigma.');
...
function runAtmosphereDeterministicConstantCasesRegression()
    ...
    assertDeterministicAtmosphereCase( ...
        "atmosphere_truth_only_constant", ...
...
function assertDeterministicAtmosphereCase( ...
    ...
    assert(~isfield(reportData, 'atmosphere'), ...
        ['reportData.atmosphere is forbidden; reportData should expose only ', ...
         'report-layout fields derived from sim.history.errors.']);
    assert(isfield(reportData, 'atmosphere_residual_by_receiver_tower_m'), ...
        'Report data must expose the flat atmosphere residual report field.');
    assert(all(abs(reportData.atmosphere_residual_by_receiver_tower_m(visible) - ...
        expectedResidual_m) < 1e-12), ...
...
function runAtmosphereComponentResidualDecompositionRegression()
    ...
    assert(~isfield(reportData, 'atmosphere'), ...
        ['reportData.atmosphere is forbidden; reportData must expose only ', ...
         'report-layout fields derived from sim.history.errors.']);
