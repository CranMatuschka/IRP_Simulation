...
% Section 2.3 item 3 legitimately declares oneWayCode/oneWayDoppler (both notAClockObservable --
% see DistributedClockGaugeContract's own ClockClaimByObservable header comment for why); the
% undeclared negative control below uses a genuinely never-declared identifier instead.
assert(strcmp(revgnss.DistributedClockGaugeContract.requireObservableClockClaimDeclared( ...
...
i_expectError_(@() revgnss.DistributedClockGaugeContract.requireObservableClockClaimDeclared( ...
    'oneWayCarrier'),'DistributedClockGaugeContract:observableClockClaimUndeclared');
...
% Section 2.4 itself added neither vocabulary; the five entries present here are Section
% 2.3.1's (coherentTwoWayCodeRange), Section 2.3.2's (firstOrderReciprocalClockTransfer),
% Section 2.3 item 3's (oneWayCode, oneWayDoppler), and Section 4.4's
% (fourTimestampClockDifference) own sanctioned widenings, each with its own dedicated adapter
% and stage-acceptance test.
assert(isequal(revgnss.DistributedLinkUpdateAdapter.AllowedObservables, ...
    ...
    'AllowedObservables must carry exactly the observables sanctioned through Section 4.4.');
assert(isequal(revgnss.DistributedLinkUpdateAdapter.RegisteredAdapterClasses, ...
    ...
    'RegisteredAdapterClasses must carry exactly the five sanctioned adapters.');
fprintf('  PASS Section 2.4 enables nothing new beyond Section 2.3.1/2.3.2/2.3-item-3/4.4''s own widenings\n');
end
