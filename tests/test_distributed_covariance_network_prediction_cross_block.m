...
% Section 3.2's own mechanical tripwires (never previously called anywhere): a source-regex
% proof that phase 5's coordinator method/name were not silently renamed, and that the
% synchronized-delivery sub-phase order stays byte-identical to its frozen declaration.
revgnss.SynchronizedDeliveryContract.requireSubPhaseOrderUnchanged();
revgnss.SynchronizedDeliveryContract.requirePhaseSixNameFrozen();
assert(any(strcmp('pairExactWhenBothEndpointsTracked', ...
    revgnss.DistributedCovarianceNetworkContract.AllowedLinkUpdateRoutingPolicies)) && ...
    any(strcmp('conservativeBoundOnly', ...
    revgnss.DistributedCovarianceNetworkContract.AllowedLinkUpdateRoutingPolicies)), ...
    'AllowedLinkUpdateRoutingPolicies must carry both the Stage 3.1 and Stage 3.2 routing words');
assert(isequal(revgnss.SynchronizedDeliveryContract.PairExactEligibleObservables, ...
    {'coherentTwoWayCodeRange'}),'PairExactEligibleObservables must stay exactly its frozen Section 3.2 value');
