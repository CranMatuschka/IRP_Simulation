...
assert(isfinite(s1.candidateImprovementRatio), ...
    'T1 FAILED: truth diagnostic improvement ratio missing');
assert(isfinite(s1.priorResidualCycles) && ...
    isfinite(s1.residualImprovementRatio), ...
    'T1 FAILED: measurement-residual improvement telemetry missing');
...
fprintf('    PASS (confidence=%s, ratio=%.3f, candErr=%.3f deg, residualImp=%.3f)\n', ...
    s1.confidenceClass, s1.ratio, ...
    s1.candidateAttitudeError_deg, s1.residualImprovementRatio);
...
if s1.residualImprovementRatio >= 1.05 && s1.ratio < 1.20
