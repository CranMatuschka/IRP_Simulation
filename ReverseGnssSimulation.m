classdef ReverseGnssSimulation < handle
    ...
    methods (Access = private)
        ...
        function setupSpaceAssetsAndReceivers(obj)
            ...
            % Single source of truth for GEO initial state.
            initialState = SpaceAsset.initialGeoState(obj.assetConfig, obj.jd0, obj.mu);
        
            % ---------------------------------------------------------------------
            % Select RX Antenna configurations before constructing SpaceAsset.
            % SpaceAsset constructor now owns Antenna object construction.
            % ---------------------------------------------------------------------
            if isfield(obj.cfg.spaceAssets, 'antennas')
                antCfgAvailable = obj.cfg.spaceAssets.antennas;
            elseif isfield(obj.cfg.spaceAssets, 'receivers')
                % Migration support only. New configs should use spaceAssets.antennas.
                antCfgAvailable = obj.cfg.spaceAssets.receivers;
            elseif isfield(obj.assetConfig, 'antennas')
                antCfgAvailable = obj.assetConfig.antennas;
            else
                error('ReverseGnssSimulation:NoAntennaConfig', ...
                    'No RX antenna configuration found. Expected spaceAssets.antennas.');
            end
            ...
            if isfield(obj.runtimeOptions, 'N_RECEIVERS') && ~isempty(obj.runtimeOptions.N_RECEIVERS)
                ...
                nRequested = min(nRequested, numel(antCfgAvailable));
        
                obj.rxConfig = antCfgAvailable(1:nRequested);
        
                for idxRx = 1:numel(obj.rxConfig)
                    obj.rxConfig(idxRx).enabled = true;
                    obj.rxConfig(idxRx).mode = "RX";
                end
            else
                if isfield(antCfgAvailable, 'enabled')
                    obj.rxConfig = antCfgAvailable([antCfgAvailable.enabled]);
                else
                    obj.rxConfig = antCfgAvailable;
                end
        
                if isfield(obj.cfg, 'numReceiversToUse')
                    obj.rxConfig = obj.rxConfig( ...
                        1:min(double(obj.cfg.numReceiversToUse), numel(obj.rxConfig)));
                elseif isfield(obj.cfg.spaceAssets, 'receiverCountPerAsset') && ...
                        ~isempty(obj.cfg.spaceAssets.receiverCountPerAsset)
                    obj.rxConfig = obj.rxConfig( ...
                        1:min(double(obj.cfg.spaceAssets.receiverCountPerAsset(1)), numel(obj.rxConfig)));
                end
            ...
            if isempty(obj.rxConfig)
                error('ReverseGnssSimulation:AntennaCount', ...
                    'At least one enabled RX Antenna is required.');
            end
            ...
            % ---------------------------------------------------------------------
            % Now build the asset config used by the constructor.
            % This is the critical fix.
            % ---------------------------------------------------------------------
            assetConfigForRun = obj.assetConfig;
            assetConfigForRun.antennas = obj.rxConfig;
            assetConfigForRun.attitudeFrame = obj.attitudeFrame;
            ...
            antennasTruth = SpaceAsset.buildAntennaArray(obj.rxConfig);
            antennasEst   = SpaceAsset.buildAntennaArray(obj.rxConfig);
            
            obj.assetTruth = SpaceAsset( ...
                obj.assetConfig.id, ...
        ...
        function setupClockAndEkf(obj)
            ...
            [obj.PhiClk, Qclk] = Clock.aggregateBiasDriftModel( ...
                obj.oscillatorConfig.h0, obj.oscillatorConfig.hm1, ...
                obj.oscillatorConfig.hm2, ...
                obj.dt, obj.c, obj.cfg.clockModel, ...
                obj.cfg.clockGaussMarkovCorrelationTime_s);
        ...
        function step(obj, k)
            ...
            trueClock_m = obj.assetTruth.clock.get_error_meters();
            trueClockDrift_mps = obj.c * obj.assetTruth.clock.total_drift_sec_per_s;
            ...
            obj.scClock = obj.assetTruth.clock;
        end
        ...
        function buildResults(obj)
            ...
            obj.results.receiver_offsets_body_m = [obj.assetTruth.antennas.offsetBody_m];
        ...
        function [y, Rrange] = makePseudoranges(obj, asset, towersEci, spaceClockBias_m, towerClockBias_m, measurementConfig)
            ants = asset.getEnabledAntennas();
        
            y = zeros(numel(ants) * size(towersEci, 2), 1);
            ...
            antennaPhaseCenterEnabled = false;
            if isfield(obj.cfg, 'errorModels') && ...
                    isfield(obj.cfg.errorModels, 'antennaPhaseCenter') && ...
                    isfield(obj.cfg.errorModels.antennaPhaseCenter, 'enabled')
                antennaPhaseCenterEnabled = logical(obj.cfg.errorModels.antennaPhaseCenter.enabled);
            end
        
            for i = 1:numel(ants)
                rrx = asset.getReceiverPositionECI(ants(i));
        
                for j = 1:size(towersEci, 2)
                    d = rrx - towersEci(:, j);
                    rhoGeom = norm(d);
        
                    antennaCorrection_m = 0.0;
        
                    if antennaPhaseCenterEnabled
                        % RX antenna correction.
                        % Current default pco_m = 0 and pcvMap = [], therefore this is
                        % exactly zero unless explicitly configured.
                        %
                        % Important:
                        %   If non-zero PCV is enabled, H must later include
                        %   d(PCV)/d(attitude).
                        u_from_rx_to_tx_eci = (towersEci(:, j) - rrx) ./ rhoGeom;
                        R_body_to_eci = asset.getRotationBodyToECI();
                        u_from_rx_to_tx_body = R_body_to_eci.' * u_from_rx_to_tx_eci;
        
                        [elev_rx_deg, az_rx_deg] = obj.losToElevationAzimuth(u_from_rx_to_tx_body);
                        antennaCorrection_m = antennaCorrection_m + ...
                            ants(i).getRangeCorrection(elev_rx_deg, az_rx_deg, [], "RX");
        
                        % TX antenna correction is intentionally zero unless the tower
                        % antenna has non-zero PCO/PCV and the local ENU projection is
                        % implemented. GroundNode.antenna defaults to zero, preserving
                        % the clock-only validation state.
                    end
        
                    y(row) = rhoGeom + antennaCorrection_m + spaceClockBias_m - towerClockBias_m(j);
        ...
        function [y, H] = predictPseudorangesWithJacobian(obj, x, asset, towersEci)
            ...
            for i = 1:numel(ants)
                [rrx, drrx_dstate, drrx_datt] = SpaceAsset.receiverPositionAndJacobians( ...
                    ants(i), state_eci, attitude_rad, obj.attitudeFrame);
                ...
                for j = 1:size(towersEci, 2)
                    ...
                    % Do not add PCO/PCV derivatives here yet.
                    % With default Antenna.pco_m = 0 and Antenna.pcvMap = [],
                    % the current zero-bias validation is mathematically invariant.
                    % Future non-zero PCV requires d(PCV)/d(attitude).
        ...
        function reportData = buildGenerateReportData(obj)
            ...
            reportData.receiver_offset_body_by_receiver_m = [obj.assetTruth.antennas.offsetBody_m];
            ...
            reportData.receiver_architecture_note = sprintf( ...
                'N=%d enabled RX Antenna phase centres on one SpaceAsset. All RX antenna measurements share the same spacecraft clock state. Ground TX antenna clocks are handled by clockGaugeMode=%s.', ...
        ...
        function [frameTable, roundtripTable] = buildFrameTransformTables(obj)
            ...
            for idxEpoch = 1:numel(epochIndex)
                ...
                rx = obj.assetTruth.getEnabledAntennas();
    ...
    methods (Access = private)
        function clockObj = makeSpaceAssetClock(obj)
            osc = obj.simConfig.clockLibrary.(obj.assetConfig.sharedClock.clockType);
        
            clockObj = Clock(osc.h0, osc.hm1, osc.hm2, obj.dt);
            clockObj.randomStream = obj.clockStream;
        ...
        function antennas = makeAntennas(~, antennaConfig)
            antennas = Antenna.empty(1, 0);
        
            for k = 1:numel(antennaConfig)
                antennas(1, k) = Antenna(antennaConfig(k));
            end
        end
    end
