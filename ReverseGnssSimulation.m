classdef ReverseGnssSimulation < handle
    ...
    methods
        ...
        function generateReport(obj)
            ...
            reportConfig.compilePdf = logical(obj.getFieldOrDefault(obj.cfg.report, 'compilePdf', true));
            reportConfig.interactivePlots = logical(obj.getFieldOrDefault(obj.cfg.report, 'interactivePlots', false));
    ...
    methods (Access = private)
        ...
        function reportData = buildGenerateReportData(obj)
            ...
            Pdiag = obj.history.covariance_diag;
            ...
            reportData.c = obj.c;
            reportData.num_towers = obj.numTowers;
            reportData.R_earth = obj.constants.earthRadius_m;
            ...
            reportData.ekf_clock_sigma_ps = sqrt(max(Pdiag(obj.idx.rxClockBias, :), 0)) ./ obj.c .* 1e12;
            reportData.true_clock_bias_ps = truthHist(obj.idx.rxClockBias, :) ./ obj.c .* 1e12;
            reportData.est_clock_bias_ps = xHist(obj.idx.rxClockBias, :) ./ obj.c .* 1e12;
            if obj.towerClockEkfEnabled()
                reportData.est_tower_clock_bias_ps = ...
                    xHist(obj.idx.towerClockBias, :) ./ obj.c .* 1e12;

                reportData.true_tower_clock_bias_ps = ...
                    truthHist(obj.idx.towerClockBias, :) ./ obj.c .* 1e12;

                reportData.est_tower_clock_drift_psps = ...
                    xHist(obj.idx.towerClockDrift, :) ./ obj.c .* 1e12;

                reportData.true_tower_clock_drift_psps = ...
                    truthHist(obj.idx.towerClockDrift, :) ./ obj.c .* 1e12;
            end
            ...
            reportData.attitude_error_deg = rad2deg( ...
                FrameGeometry.wrapToPi(xHist(obj.idx.att, :) - truthHist(obj.idx.att, :)));
            reportData.attitude_sigma_deg = rad2deg(sqrt(max(Pdiag(obj.idx.att, :), 0)));
            ...
            reportData.angular_velocity_error_degps = rad2deg(xHist(obj.idx.omega, :) - truthHist(obj.idx.omega, :));
            reportData.angular_velocity_sigma_degps = rad2deg(sqrt(max(Pdiag(obj.idx.omega, :), 0)));
            ...
            reportData.final_angular_velocity_error_norm_degps = norm(reportData.angular_velocity_error_degps(:, end));

            reportData.final_H_pos_rank = obj.history.H_pos_rank_history(end);
            reportData.final_H_att_rank = obj.history.H_att_rank_history(end);
            reportData.final_H_pos_att_clock_rank = obj.history.H_pos_att_clock_rank_history(end);
            reportData.final_H_pos_column_norm = obj.history.H_pos_column_norm_history(:, end);
            ...
            reportData.H_rank_to_state_dim_history = obj.history.H_rank_history ./ max(obj.stateDim, 1);
            reportData.H_state_deficiency_history = obj.stateDim - obj.history.H_rank_history;
            reportData.final_H_rows = obj.history.measurement_count(end);
            reportData.final_H_columns = obj.stateDim;
            reportData.final_H_rank_to_state_dim = reportData.final_H_rank / max(obj.stateDim, 1);
            reportData.final_H_state_deficiency = obj.stateDim - reportData.final_H_rank;
            
            reportData.true_ground_clock_bias_ps = obj.history.ground_clock_true_m ./ obj.c .* 1e12;
            reportData.tower_clock_correction_s = obj.history.ground_clock_correction_m ./ obj.c;
            reportData.tower_clock_correction_residual_s = obj.history.ground_clock_residual_m ./ obj.c;
            reportData.tower_clock_correction_sigma_s = ones(obj.numTowers, obj.numSteps) .* sqrt(obj.groundClockResidualVariance_m2()) ./ obj.c;
            ...
            reportData.receiver_offset_body_by_receiver_m = obj.receiverOffsetsBody_m;
            ...
            reportData.used_tower_count = obj.history.visible_tower_count;
            reportData.measurementNoiseEnabled = obj.measurementModel.measurementNoiseEnabled();
            reportData.numerical_measurement_sigma_floor_m = obj.measurementModel.effectiveNumericalMeasurementSigma_m();
            reportData.enableElevationMask = obj.measurementModel.elevationMaskEnabled();
            reportData.elevationMask_deg = obj.measurementModel.elevationMaskDeg();
            if obj.towerClockEkfEnabled()
            ...
            reportData.receiver_architecture_note = sprintf( ...
                'N=%d onboard receiver phase centres share one receiver oscillator. Lever arms are Body-frame vectors rotated by q_BI into ECI.', obj.numReceivers);
            if obj.towerClockEkfEnabled()
                reportData.signal_model_note = ['Truth code pseudorange: P = rho(r_sc_I + C_BI l_a_B, r_g_I) + b_rx_true - b_g_true + d_truth + noise. ' ...
                ...
                reportData.signal_model_note = ['Truth code pseudorange: P = rho(r_sc_I + C_BI l_a_B, r_g_I) + b_rx_true - b_g_res_true + d_truth + noise. ' ...
            ...
            reportData.attitude_observability_note = ['Attitude remains in the EKF for every receiver count. Its pseudorange sensitivity is -u^T C_BI skew(l_a_B); zero lever arms produce zero attitude sensitivity naturally.'];
            reportData.attitude_filter_note = 'MEKF-style attitude update uses a body-frame small-angle error injected multiplicatively into q_BI.';
            reportData.state_vector_table = obj.buildStateVectorReportTable();
            ...
            nRows = obj.numReceivers * obj.numTowers;
            receiverR2 = obj.cfg.measurement.pseudorangeSigma_m^2 * ...
                double(obj.measurementModel.measurementNoiseEnabled());
            groundR2 = obj.groundClockResidualVariance_m2();
            actualR2 = obj.measurementModel.measurementVariance( ...
                obj.towerClockEkfEnabled(), obj.groundClockResidualVariance_m2());
            reportData.R_receiver_m2 = ones(nRows, obj.numSteps) * receiverR2;
            reportData.R_tower_clock_m2 = ones(nRows, obj.numSteps) * groundR2;
            reportData.R_atmosphere_m2 = zeros(nRows, obj.numSteps);
            reportData.R_hardware_m2 = zeros(nRows, obj.numSteps);
            reportData.R_multipath_m2 = zeros(nRows, obj.numSteps);
            reportData.R_numerical_regularization_m2 = ones(nRows, obj.numSteps) * max(actualR2 - receiverR2 - groundR2, 0.0);
            reportData.R_total_m2 = reportData.R_receiver_m2 + reportData.R_tower_clock_m2 + reportData.R_numerical_regularization_m2;

            reportData.starting_position_table = obj.buildStartingPositionTable();
            obs = obj.observabilityDiagnostics();
            reportData.final_observability_rank = obs.rank;
            reportData.weak_observability_state_names = obs.weakStateNames;
            reportData.observability_note = sprintf('Column-normalized accumulated pseudorange observability rank is %d of %d states. Weak states: %s.', ...
                obs.rank, obj.stateDim, strjoin(string(obs.weakStateNames), ', '));

            selectedOsc = obj.simConfig.clockLibrary.(char(obj.assetConfig.clock.clockType));
            selectedClock = Clock(selectedOsc.h0, selectedOsc.hm1, selectedOsc.hm2, obj.dt);
            nAllan = floor(obj.simConfig.validation.allanValidationSamples);
            
            reportData.tau_profile_s = obj.validTauForSamples(obj.simConfig.validation.tauProfile_s, obj.dt, nAllan);
            
            clockValidationList = [{selectedClock}, GroundNode.clocks(obj.towers)];
            clockNames = ["SpaceAsset RX", obj.towerNames];
            nClock = numel(clockValidationList);
            clockAdev1s = NaN(1, nClock);
            
            for idxClock = 1:nClock
                [tauThis_s, adevThis, sigmaThis, edfThis] = ...
                    obj.runClockAllanValidation(clockValidationList{idxClock}, ...
                    obj.simConfig.validation.tauSimulation_s, obj.dt, nAllan);
                ...
                clockAdev1s(idxClock) = clockValidationList{idxClock}.theoreticalAllanDeviation(1.0);
            end
            ...
            reportData.source_references = {sprintf('Generated by %s.', obj.entryPointName)};
        end
        ...
        function tableOut = buildStartingPositionTable(obj)
            ...
            if ~isempty(obj.initialTruth0)
                r0_I = obj.initialTruth0(obj.idx.pos);
            else
                r0_I = obj.truthAsset.pos_ECI_m;
            end
            
            append("SpaceAsset", obj.assetConfig.name, "ECI initial center of mass", ...
                sprintf("X %.6g m", r0_I(1)), ...
                sprintf("Y %.6g m", r0_I(2)), ...
                sprintf("Z %.6g m", r0_I(3)));
            for rx = 1:obj.numReceivers
                off = obj.receiverOffsetsBody_m(:, rx);
                append("Receiver", obj.receiverNames(rx), "Body lever arm", ...
                    sprintf("X %.6g m", off(1)), sprintf("Y %.6g m", off(2)), sprintf("Z %.6g m", off(3)));
            end
            for twr = 1:obj.numTowers
                tc = obj.activeTowerConfig(twr);
                append("Tower", obj.towerNames(twr), "Fixed geodetic/ECEF source", ...
                    sprintf("Lat %.6g deg", tc.lat_deg), sprintf("Lon %.6g deg", tc.lon_deg), sprintf("Alt %.6g m", tc.alt_m));
            end
            tableOut = table(objectType, objectName, positionFrame, position1, position2, position3, ...
                'VariableNames', {'ObjectType','Name','Frame','Position1','Position2','Position3'});
            function append(typeValue, nameValue, frameValue, pos1, pos2, pos3)
        ...
        function tableOut = buildObservationMatrixDiagnosticsTable(obj)
            %BUILDOBSERVATIONMATRIXDIAGNOSTICSTABLE Report dimensions/rank of H.
            %
            % H rows are the scalar measurement equations used in the EKF update.
            % H columns are the EKF error-state dimension.
        
            measCount = obj.history.pseudorange_measurement_count(:);
            ...
            gaugeRows = 0;
            if obj.towerClockEkfEnabled()
                % appendTowerClockGaugeConstraint adds one bias-gauge and one
                % drift-gauge constraint.
                gaugeRows = 2;
            end
        
            finalHRows = finalPseudorangeRows + gaugeRows;
            hColumns = obj.stateDim;
        
            hRank = obj.getHistoryVector('H_rank_history');
            ...
            hPosRank = obj.getHistoryVector( 'H_pos_rank_history');
            hAttRank = obj.getHistoryVector( 'H_att_rank_history');
            hCombinedRank = obj.getHistoryVector( 'H_pos_att_clock_rank_history');
        
            finalPosRank = obj.lastFinite(hPosRank);
            finalAttRank = obj.lastFinite(hAttRank);
            finalCombinedRank = obj.lastFinite(hCombinedRank);
            ...
            Value = [ ...
                obj.formatNumber(finalHRows); ...
                obj.formatNumber(hColumns); ...
                obj.formatNumber(finalRankH); ...
                obj.formatNumber(finalRankDeficiency); ...
                obj.formatNumber(minRankH); ...
                obj.formatNumber(maxRankH); ...
                obj.formatNumber(finalPseudorangeRows); ...
                sprintf("%.2f", meanPseudorangeRows); ...
                obj.formatNumber(minPseudorangeRows); ...
                obj.formatNumber(maxPseudorangeRows); ...
                obj.formatNumber(gaugeRows); ...
                obj.formatNumber(finalPosRank); ...
                obj.formatNumber(finalAttRank); ...
                obj.formatNumber(finalCombinedRank) ...
                ];
        ...
        function v = getHistoryVector(obj, fieldName)
            if isfield(obj.history, fieldName) && ~isempty(obj.history.(fieldName))
                v = obj.history.(fieldName)(:);
            else
                v = NaN;
            end
        end
        ...
        function tableOut = buildStateVectorReportTable(obj)
            ...
            Symbol(1:14) = baseSymbol;
            Unit(1:14) = baseUnit;
            Description(1:14) = names(1:14);
            ...
            if obj.towerClockEkfEnabled()
                ...
                for twr = 1:obj.numTowers
                    ...
                    Description(row) = sprintf('%s tower clock bias', obj.towerNames(twr));
                    Unit(row) = "m";
                    DynamicCouplingNote(row) = "Estimated relative to mean ground-network clock gauge";
                    row = row + 1;

                    Symbol(row) = sprintf('delta bdot_g,%d', twr);
                    Description(row) = sprintf('%s tower clock drift', obj.towerNames(twr));
                    Unit(row) = "m/s";
            ...
            tableOut = table(Index, Symbol, Description, Unit, ...
                DynamicCouplingNote);
        end
        ...
        function tableOut = buildHFactorObservabilityTable(obj)
            %BUILDHFACTOROBSERVABILITYTABLE Explain which EKF state blocks H observes.
            %
            % Direct H-rank is instantaneous. Accumulated rank uses the stored
            % observability normal matrix sum_k (H_k Phi_k)'(H_k Phi_k), so it
            % includes dynamic coupling through F.
        
            kFinal = find(isfinite(obj.history.H_rank_history), 1, 'last');
            if isempty(kFinal)
                kFinal = obj.numSteps;
            end
        
            hRows = obj.history.H_row_count_history(kFinal);
            hCols = obj.history.H_column_count_history(kFinal);
            hRank = obj.history.H_rank_history(kFinal);
            hDef = hCols - hRank;
            ...
            appendBlock("receiver clock bias db_rx", obj.idx.rxClockBias, ...
                "1", ...
                double(obj.history.H_rx_clock_bias_column_norm_history(kFinal) > 0.0), ...
                "Directly observed as a common range offset across all pseudoranges.");
            
            appendBlock("receiver clock drift dbdot_rx", obj.idx.rxClockDrift, ...
                "0", ...
                0, ...
                "Not directly observed by pseudorange. It becomes observable only because clock drift propagates into future clock bias.");
            if obj.towerClockEkfEnabled()
                visibleTowers = obj.history.visible_tower_count(kFinal);
        
                appendBlock("tower clock biases db_g", obj.idx.towerClockBias, ...
                    "-1 for the transmitting tower, plus mean-clock gauge row", ...
                    min(obj.numTowers, visibleTowers + 1), ...
                    "Directly observed only for visible towers. The mean-clock gauge fixes the otherwise arbitrary network clock reference.");
                
                appendBlock("tower clock drifts dbdot_g", obj.idx.towerClockDrift, ...
            ...
            appendBlock("full instantaneous H", 1:obj.stateDim, ...
                "all active H columns", ...
                hRank, ...
                sprintf("Final H has %d rows, %d columns, rank %d, deficiency %d. This is instantaneous rank only.", ...
                hRows, hCols, hRank, hDef));
            ...
            function appendBlock(blockName, idxBlock, hFactorText, directRank, interpretationText)
                ...
                DirectRankFinalEpoch(end + 1, 1) = string(formatRank(directRank, numel(idxBlock)));
                AccumulatedDynamicRank(end + 1, 1) = string(formatRank(accumulatedRank, numel(idxBlock)));
