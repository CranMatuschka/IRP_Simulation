...
%   revgnss.ReportLabel so no raw CamelCase config string reaches the reader.
    ...
    % Relativistic clock-rate offset. CORRECTED 2026-08-09: the previous text asserted the
    % offset "is absorbed by the estimated clock-drift state ... leaves zero estimation
    % residual". That was measurably false as implemented -- the truth range carried the
    % ramp while the truth Doppler did not, so the 2-state clock could not represent it and
    % the excess leaked into position (13.1 m on an OCXO clock Q). Both sides are now
    % consistent and a matching model-side correction is available.
    fprintf(fid, ['Relativistic clock-rate offset (gravitational + special-relativistic, ' ...
        ...
        'a gated truth-side term on the receiver clock (default off), present in BOTH the truth ' ...
        'range and the truth range-rate. When the model-side correction is enabled the estimator ' ...
        'applies the same published constant, so the clock states carry only the oscillator ' ...
        'residual; with it disabled the offset must instead be absorbed by the estimated clock ' ...
        'states, which a two-state clock can only do exactly once its drift state holds the full ' ...
        'rate. Only an eccentric orbit''s periodic term is unmodelled.\\\\\n']);
    % Receiver clock-drift +-3sigma coverage is observability-limited (documented).
    ...
    % Clock provenance. The idealised-vs-realistic caveat is retired: there is one table.
    fprintf(fid, ['Receiver and tower oscillator h-coefficients are Table 2.1 of ' ...
        'Winkel (2003), the project primary source; there is no second, more optimistic ' ...
        'table to select. Coefficients can be overridden per run via ' ...
        'clock.customOscillators, which is recorded in the resolved configuration.\\\\\n']);
    fprintf(fid, '\\bottomrule\n\\end{tabular}\n');
