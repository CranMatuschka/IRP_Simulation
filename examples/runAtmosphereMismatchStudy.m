...
assertClose(reportData.mean_deterministic_atmosphere_model_residual_m, ...
    expectedDeterministicResidual_m, 1e-9, ...
    'reported deterministic atmosphere residual');
assertClose(reportData.model_atmosphere_residual_variance_m2, ...
    expectedModelVariance_m2, 1e-12, ...
    'model atmosphere covariance variance');

assert(isfield(results, 'errors') && ...
    isfield(results.errors, 'atmosphere'), ...
    'Saved results structure must expose canonical atmosphere error history.');
assert(exist(resultFile, 'file') == 2, ...
    'Expected saved results file was not created: %s', resultFile);
assert(any(reportData.atmosphere_summary_table.Quantity == ...
    "R covariance structure"), ...
    'Atmosphere report summary must expose R covariance structure.');
...
fprintf('Model covariance variance in R: %.3f m^2 (%s)\n', ...
    ...
    char(reportData.model_atmosphere_covariance_structure));
