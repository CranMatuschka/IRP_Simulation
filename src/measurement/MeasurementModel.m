        ...
        function [extraTruth_m, extraModel_m, budget] = ...
            ...
            if obj.useHardwareDelay
                ...
                budget.hardware = obj.withTruthModel( ...
                    budget.hardware, hardwareTruth_m, 0.0);
                budget.hardware.diagnostics = struct( ...
                    'tx_configured_delay_m', obj.txHardwareDelay_m, ...
                    'tx_tower_specific_delay_m', towerSpecificTxDelay_m, ...
                    'rx_configured_delay_m', obj.rxHardwareDelay_m);
            end

            if obj.useMultipathDelay
                budget.multipath = obj.withTruthModel( ...
                    budget.multipath, obj.multipathDelay_m, 0.0);
                budget.multipath.diagnostics = struct( ...
                    'configured_delay_m', obj.multipathDelay_m);
            end
            ...
            if obj.useAntennaDelay
                ...
                budget.antenna = obj.withTruthModel( ...
                    budget.antenna, antennaTruth_m, 0.0);
                budget.antenna.diagnostics = struct( ...
                    'configured_delay_m', obj.antennaDelay_m, ...
                    'tx_correction_m', txCorr_m, ...
                    'rx_correction_m', rxCorr_m);
            end

            if obj.useSagnacCorrection
                budget.legacy_sagnac = obj.withTruthModel( ...
                    budget.legacy_sagnac, obj.sagnacCorrection_m, 0.0);
                budget.legacy_sagnac.diagnostics = struct( ...
                    'configured_delay_m', obj.sagnacCorrection_m, ...
                    'note', "Legacy truth-only Sagnac term; propagation diagnostics also log this component.");
            end

            budget.tower_survey.diagnostics = struct( ...
                'note', "Tower survey range-effect scaffold is zero until true and model tower positions are separated.");
            ...
            budget.total.diagnostics = struct( ...
                'note', "Current non-atmospheric model corrections are zero unless a future calibrated model is added.");
        end
