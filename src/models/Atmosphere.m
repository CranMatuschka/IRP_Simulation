classdef Atmosphere < handle
    ...
    properties (SetAccess = private)
        ...
        ionosphereProvider = []
        troposphereProviderType string = "none"
        troposphereProvider = []
    ...
    methods
        function obj = Atmosphere(atmosphereCfg, constants, role)
            ...
            obj.troposphereProviderType = obj.normalizeChoice( ...
                obj.getFieldOrDefault( ...
                    obj.cfg, ...
                    'troposphereProviderType', ...
                    obj.getFieldOrDefault(obj.cfg, 'troposphereProvider', "none")), ...
                ["none", "profile", "era5"], ...
                'troposphereProviderType');

            obj.troposphereProvider = TroposphereProfileProviderFactory.create( ...
                obj.troposphereProviderType, ...
                obj.dataRoot, ...
                obj.cfg, ...
                obj.role);
    ...
    methods (Access = private)
        ...
        function [delay_m, tropoResult] = troposphereDelayMeters( ...
            ...
            switch obj.troposphereModel
                ...
                case "profile"
                    [delay_m, tropoResult] = ...
                        obj.profileTroposphereDelayWithMetadataMeters( ...
                        groundNode, ...
                        elevation_deg, ...
                        datetimeUtc);
        ...
        function [delay_m, tropoResult] = ...
                profileTroposphereDelayWithMetadataMeters( ...
                obj, groundNode, elevation_deg, datetimeUtc)

            tropoResult = obj.emptyTroposphereResult( ...
                datetimeUtc, groundNode);

            if isempty(obj.troposphereProvider) || ...
                    ~obj.troposphereProvider.isAvailable()
                delay_m = obj.handleMissingTroposphereData( ...
                    'Troposphere profile provider is not available.');
                tropoResult.message = ...
                    "Troposphere profile provider is not available.";
                return;
            end

            profileResult = obj.troposphereProvider.profileAt( ...
                datetimeUtc, ...
                groundNode.lat_deg, ...
                groundNode.lon_deg, ...
                groundNode.alt_m);

            if ~isfield(profileResult, 'valid') || ~profileResult.valid
                delay_m = obj.handleMissingTroposphereData( ...
                    'Troposphere profile data are unavailable at the requested time/location.');
                if isfield(profileResult, 'message')
                    tropoResult.message = string(profileResult.message);
                else
                    tropoResult.message = ...
                        "Troposphere profile data are unavailable.";
                end
                return;
            end

            pressure_hPa = double(profileResult.pressure_hPa);
            temperature_K = double(profileResult.temperature_K);

            if isfield(profileResult, 'waterVaporPressure_hPa') && ...
                    isfinite(profileResult.waterVaporPressure_hPa)
                waterVaporPressure_hPa = ...
                    double(profileResult.waterVaporPressure_hPa);
            else
                waterVaporPressure_hPa = obj.waterVaporPressure_hPa( ...
                    temperature_K, ...
                    double(profileResult.relativeHumidity_fraction));
            end

            validateattributes(pressure_hPa, {'numeric'}, ...
                {'real', 'finite', 'scalar', 'positive'}, ...
                mfilename, 'profilePressure_hPa');

            validateattributes(temperature_K, {'numeric'}, ...
                {'real', 'finite', 'scalar', '>', 150.0}, ...
                mfilename, 'profileTemperature_K');

            validateattributes(waterVaporPressure_hPa, {'numeric'}, ...
                {'real', 'finite', 'scalar', 'nonnegative'}, ...
                mfilename, 'profileWaterVaporPressure_hPa');
            ...
            if heightCorrection <= 0.0
                error('Atmosphere:InvalidProfileHeightCorrection', ...
                    'Profile troposphere height correction must be positive.');
            end
            ...
            tropoResult.valid = true;
            tropoResult.message = ...
                "Profile-provider hydrostatic plus wet delay.";

            tropoResult.pressure_hPa = pressure_hPa;
            tropoResult.temperature_K = temperature_K;

            if isfield(profileResult, 'relativeHumidity_fraction')
                tropoResult.relativeHumidity_fraction = ...
                    double(profileResult.relativeHumidity_fraction);
            end

            tropoResult.waterVaporPressure_hPa = waterVaporPressure_hPa;

            tropoResult.zenithHydrostaticDelay_m = ...
                zenithHydrostaticDelay_m;

            tropoResult.zenithWetDelay_m = ...
                zenithWetDelay_m;
            ...
            tropoResult.slantWetDelay_m = ...
                slantWetDelay_m;

            tropoResult.total_m = delay_m;

            tropoResult.providerType = string(profileResult.providerType);

            if isfield(profileResult, 'source')
                tropoResult.source = string(profileResult.source);
            end

            if isfield(profileResult, 'metadata')
                tropoResult.metadata.profile = profileResult.metadata;
            end
        end

        function delay_m = handleMissingTroposphereData(obj, messageText)
            if obj.missingDataPolicy == "error"
                error('Atmosphere:MissingTroposphereProfileData', ...
                    '%s', messageText);
            end
        ...
        function validateConfiguration(obj)
            ...
            if isempty(obj.troposphereProvider) || ...
                    ~isa(obj.troposphereProvider, 'TroposphereProfileProvider')
                error('Atmosphere:InvalidTroposphereProvider', ...
                    ['troposphereProvider must implement ', ...
                     'TroposphereProfileProvider.']);
            end
