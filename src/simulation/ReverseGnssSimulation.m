classdef ReverseGnssSimulation < handle
    ...
    methods (Access = private)
        ...
        function step(obj, k)
            ...
            epoch.groundResidual_m = groundResidualTruth_m;
            epoch.groundTrue_m = groundTrue_m;
            epoch.groundCorrection_m = groundCorrection_m;

            truth = obj.measurementModel.makePseudoranges( ...
                epoch.jd, epoch.datetimeUtc, epoch.towersEci, groundResidualTruth_m, ...
                obj.truthAsset, obj.towerClockEkfEnabled(), ...
                GroundTimingNetwork.residualVariance_m2(obj.cfg, obj.c));

            model = obj.measurementModel.predictPseudorangesWithJacobian( ...
                epoch.jd, epoch.datetimeUtc, epoch.towersEci, groundResidualModel_m, ...
            ...
            filterStats = struct( ...
                'innovation', innovationRange, ...
                'postfit', postfitRange, ...
                'S', S, ...
                'H', Hupdate, ...
                'nisValue', nisValue, ...
                'P', obj.ekf.P, ...
                'Pdiag', diag(obj.ekf.P), ...
                'estimateVector', obj.physicalEstimateVector(), ...
                'truthVector', obj.physicalTruthVector());
        ...
        function epoch = makeEpochContext(obj, k)
            epoch = struct();
            epoch.index = k;
            epoch.time_s = obj.time_s(k);
            epoch.jd = obj.jd0 + epoch.time_s / 86400.0;
            epoch.datetimeUtc = ...
                obj.simConfig.simulation.startUtc + seconds(epoch.time_s);
            epoch.towersEci = GroundNode.positionsECI(obj.towers, epoch.jd);
        end
