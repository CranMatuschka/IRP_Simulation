...
| Item | Value |
|------|-------|
| Stage | **35** — Single-Asset Attitude Evidence Report v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 35; **full suite not run**) |
...
### Stage 35 validation procedure

Stage 35 adds a scenario-level attitude evidence report. It collects Stages 31–34
diagnostics (observability audit, receiver geometry, Euler convention, Jacobian audit)
together with run-level truth/estimate histories and covariance data. This is evidence
reporting only: no estimator redesign, no attitude accuracy claim, no integer ambiguity
fixing, no quaternion EKF. The simulation duration remains 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','35')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 35), force `test_stage35*` if present
