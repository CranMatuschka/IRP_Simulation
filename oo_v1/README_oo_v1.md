...
| Item | Value |
|------|-------|
| Stage | **58** — EKF Two-Body/J2 Dynamics Prediction v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 58; **full suite not run**) |
...
### Stage 58 validation procedure

Stage 58 adds optional EKF translational prediction using inertial two-body/J2 dynamics.
The EKF state remains ECEF. `EkfDynamicsPredictor` converts the ECEF position/velocity to
an inertial-like frame using a constant-Earth-rotation model, propagates with
`OrbitDynamics.rk4Step` (two-body or J2), and converts back to ECEF. A 6×6
finite-difference translational STM is used in the EKF prediction step.
`FrameTimeUtils` is extended with full ECEF/inertial state transforms.
Default mode is `constantVelocity` (backward compatible). All-toggle validation
exercises `j2` mode. No drag, SRP, third bodies, EOP/IERS, relativistic clock model,
or precise orbit products. Integer fixing, LAMBDA/MLAMBDA, and false-fix-risk control
are NOT implemented in v1. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','58')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 58), force `test_stage58*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including J2 dynamics mode, EKF innovation accounting, LinkGeometry migration, plugin registry, arc consistency enforcement, arc-separated ambiguities, carrier arc evidence, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 58 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 58, default count 4).
...
**Current stage:** Stage 58 — EKF Two-Body/J2 Dynamics Prediction v1

Stage 58 adds optional EKF translational prediction using inertial two-body/J2 dynamics.
`EkfDynamicsPredictor` propagates the ECEF state via constant-Earth-rotation frame transforms
and `OrbitDynamics.rk4Step`. Default mode is `constantVelocity` (backward compatible).
No drag, SRP, third bodies, EOP/IERS, relativistic clock model, integer fixing, LAMBDA/MLAMBDA,
calibrated phase-bias products, or false-fix-risk control are implemented in v1.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 58, default count 4). Full suite is **NOT RUN**.
