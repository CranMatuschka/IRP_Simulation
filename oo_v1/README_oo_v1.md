...
| Item | Value |
|------|-------|
| Stage | **76** — Dimension-General Configuration and Dual-Frequency Baseline Attitude AR |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 76; **full suite not run**) |
...
### Stage 76 validation procedure

Stage 76 centralizes signal frequency configuration, hardens the dimension contract for N towers
and N receiver phase centres, extends baseline attitude AR from raw L1-only to a raw L1+L2
integer-pair strategy with wide-lane consistency screening, and explicitly guards unsupported
multi-space-asset estimation. Stages 72/73/74/75 are fully preserved.

**Scientific additions (Stage 76 — dimension contract and dual-frequency AR):**

*Central signal list* — `cfg.signals.names`, `cfg.signals.frequencyHz`, `cfg.signals.enabledMask`
are populated by `ConfigFactory.finalizeConfig()` from `twoFrequency.enable`. All frequency-sensitive
code derives lambda from this central catalog. No duplicate hard-coded L1/L2 frequency constants.

*Vector frequency toggles* — `cfg.measurements.code.enabledByFrequency`, `cfg.measurements.carrier.enabledByFrequency`,
`cfg.estimator.diffAtt.ambiguityResolution.enabledByFrequency`. `[true true]` = L1+L2; `[true false]` = L1 only.
No L2 rows generated when toggle is false.

*N towers dimension-supported* — loops and diagnostics use actual `nTowers` from `sim.nTowers`.
Default 5. No hidden fixed-5 assumption.

*N receivers dimension-supported* — lever-arm geometry valid for any `nReceivers ≥ 2` up to 4
(auto cross-pattern) or any count with custom arms. Active baseline count =
`nTowers × (nReceivers - 1)` for reference-receiver mode. Default: 5 × 3 = 15 baselines.

*Multi-asset guard* — `nSpaceAssets > 1` triggers an error (or `disableWithWarning` per policy).
No silent asset truncation. `multiAssetSupported = false` always.

*Raw L1+L2 integer-pair AR* — when `enabledByFrequency = [true true]`, `BaselineCarrierAmbiguityResolver`
runs a joint search over (N1, N2) with separable cost J = J1(N1) + J2(N2), combined ratio test,
per-frequency RMS and float-distance gates, and a wide-lane consistency gate
`|N_WL_float - (N1_best - N2_best)| < 0.5 cycles`. Wide-lane is used as a consistency screening
gate only — not full WL/NL fixing. On success: `ambiguityStatus = 'fixedDualFrequencyRaw'`;
on L2 failure: `'fixedL1Only'` fallback. Both L1 and L2 EKF attitude rows generated for
`fixedDualFrequencyRaw` baselines.

*Carrier-IF integer fixing* — explicitly **unsupported** (B_IF = α·λ1·N1 + β·λ2·N2 is not an
integer on a single wavelength). `carrierIfIntegerFixing = false` always.

*Differential ionosphere in baseline AR* — neglected for short receiver baselines
(`differentialIonosphereInBaselineAr = 'neglectedShortBaselineV1'`).

*Phase-bias status* — `syntheticKnownZero` for this synthetic simulation (zero hardware biases
by construction). Real calibrated phase-bias products are not implemented.

*Global AR classifications (Stage 76)* — `fixedDualFrequencyRawAll`, `fixedL1OnlyAll`,
`fixedMixedFrequency` added alongside Stage 75 values.

**Summary fields added (Stage 76):**
`signalNames`, `signalFrequenciesHz`, `signalEnabledMask`, `signalMode`,
`codeEnabledByFrequency`, `carrierEnabledByFrequency`, `attitudeArEnabledByFrequency`,
`attitudeArMode`, `attitudeArSignalMode`, `attitudeArFrequenciesUsed`,
`wideLaneScreeningEnabled`, `carrierIfIntegerFixing`, `differentialIonosphereInBaselineAr`,
`nTowers`, `nReceivers`, `nReceiverBaselinesPerTower`, `nActiveDiffAttBaselines`,
`multiAssetSupported`, `multiAssetRequested`, `nSpaceAssetsSupported`, `dimensionContractStatus`,
`nBaselineArFixedDualFrequency`, `nBaselineArFixedL1Only` — all in existing
Simulation Physics table, no new report chapter.

**(A) Stage 72/73/74/75 baselines preserved** — `truthHistoryProductNoisy`, compensated slip
detection, block covariance, per-baseline classification. Performance and report structure unchanged.
...
setenv('OO_V1_VALIDATION_STAGE','76')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 76), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles, dual-freq L1+L2 AR, wide-lane screening
3. Verify the PDF: scientific sections present, Numerical Summary is final section
4. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 76, default count 4).
...
**Current stage:** Stage 74 — Covariance and Shared-Error Consistency

Stage 74 adds block covariance for code rows sharing the same tower clock product error.
See the Stage 74 validation procedure above for the full description.
