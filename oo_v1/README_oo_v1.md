...
| Item | Value |
|------|-------|
| Stage | **79** — Final Central Configuration Lock |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Main-script validation gate (4 selected tests, seed 79; **full suite not run**) |
...
### Stage 79 validation procedure

Stage 79 is the final central-configuration cleanup stage. `ConfigFactory.finalizeConfig()`
is the final validator/normalizer: signal masks are canonical, legacy aliases are derived,
severe contradictions error clearly, and no active configuration path silently falls back.

**Central configuration lock (Stage 79):**

*Signal masks are canonical* — `cfg.signals.names`, `cfg.signals.enabledMask`,
`cfg.measurements.code.enabledByFrequency`, `cfg.measurements.carrier.enabledByFrequency`,
`cfg.measurements.doppler.enabledByFrequency`, and
`cfg.estimator.diffAtt.ambiguityResolution.enabledByFrequency` are the frequency owners.
`cfg.signals.twoFrequency.enable`, `cfg.measurements.code.l2Rows.enable`, and
`cfg.measurements.carrier.l2EkfRows.enable` are derived compatibility aliases.

*SignalCatalog consumes canonical masks* — carrier signal rows are selected from the finalized
signal masks; invalid carrier signal indices error and never silently fall back to L1.

*Multi-asset truncation is removed entirely* — active `oo_v1` remains one estimated space asset.
If multiple assets or `nSpaceAssets>1` are requested, the active scenario errors instead of
truncating.

*Receiver geometry ownership is explicit* — user-supplied `3 x nReceivers` lever arms are
preserved and validated. If none are supplied, `ScenarioPresets.singleAssetCarrierAttitude`
uses `ReceiverGeometry.defaultLeverArms(nReceivers)`; the default active run remains four
receivers in the cross-pattern geometry.

*Clock/slip/AR/orbit owners are documented* — tower product clocks are owned by
`cfg.clocks.tower.product.*`, compensated carrier slip handling by `cfg.carrierSlip.*`,
attitude ambiguity-resolution frequency ownership by
`cfg.estimator.diffAtt.ambiguityResolution.enabledByFrequency`, and active orbit/dynamics by
`ScenarioPresets` (`twoBodyRk4` truth with matched `twoBody` EKF).

Existing physics is preserved: Stage 72 product-clock handling, Stage 73 compensated slip
detection, Stage 74 shared code covariance blocks, Stage 75/76 raw dual-frequency baseline
attitude ambiguity classification, and Stage 78 frequency centralization. The compact report
structure is preserved and still ends with Numerical Summary.

**Summary fields (Stage 79):**
`centralConfigAuditStatus`, `signalConfigOwner`, `frequencyHardcodeAuditStatus`,
`legacySignalAliasStatus`, `receiverGeometryOwner`, `multiAssetTruncationGuard`,
`clockConfigOwner`, `slipConfigOwner`, `ambiguityConfigOwner`, `orbitConfigOwner`,
`centralConfigWarnings`, `centralConfigErrors` — surfaced in existing report summary/config
tables, no new report chapter.
...
setenv('OO_V1_VALIDATION_STAGE','79')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 validation-gate tests (seed 79), stop before report generation if any selected test fails
...
Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 79, default count 4).
...
**Current stage:** Stage 79 — Final Central Configuration Lock

Stage 79 locks central configuration ownership: canonical signal masks drive downstream
frequency toggles, SignalCatalog has no unsafe L1 fallback, multi-asset truncation is removed,
receiver geometry ownership is explicit, and existing physics/report structure is preserved.
See the Stage 79 validation procedure above for the full description.
