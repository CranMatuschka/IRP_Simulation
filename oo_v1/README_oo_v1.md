...
| Item | Value |
|------|-------|
| Stage | **74** — Covariance and Shared-Error Consistency |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 74; **full suite not run**) |
...
### Stage 74 validation procedure

Stage 74 adds code-row block covariance for shared tower clock product errors. Carrier
tower-clock product covariance remains simplified (float ambiguity absorbs constant arc bias only).
Doppler clock product covariance remains simplified v1. NIS interpretation is partial/mixed.
Full suite NOT RUN. No real CLK/RINEX/SP3/ANTEX/IONEX ingestion. Single-asset one-way setup unchanged.

**Scientific fix (Stage 74 — shared-error block covariance for code rows):**
Before Stage 74, all code rows were treated as independent in `R` (diagonal only). This is wrong
because all rows from the same tower at the same product epoch share the **same tower clock product
error** `δb_twr ~ N(0, σ_twr²)`. Treating this shared error as per-row independent inflates
effective degrees of freedom and makes NIS optimistically small.

Stage 74 implements the correct block structure:
```
R_ij = σ_twr²           for same-tower pairs (i ≠ j)
R_ii = σ_tracking_i²  + σ_twr²    (diagonal unchanged)
```
Equivalently: `R = diag(σ_tracking²) + Σ_t(σ_t² · J_t)` where `J_t` is the all-ones block
for tower `t`'s rows. This is symmetric positive definite whenever all `σ_tracking > 0`.

**Carrier R policy:** unchanged. A constant tower clock product error over a carrier arc is
absorbed by the float ambiguity state; Stage 74 does not add time-varying product error
covariance to carrier `R` because modelling that correctly requires per-arc product-error
time correlation. The limitation is explicitly documented and reported.

**Doppler R policy:** simplified v1 unchanged. No tower clock drift sigma block added.

**NIS interpretation:** partially covariance-aware. Code tower-clock product correlations
are now represented in `R`. Carrier and Doppler product correlations remain diagonal-only.
NIS is not fully chi-square consistent until all measurement types have consistent `R`.

**Diagnostics fields added:**
`covarianceMode`, `codeTowerClockBlockCovarianceApplied`, `nCodeClockCovarianceBlocks`,
`meanCodeClockBlockSize`, `maxCodeClockBlockSize`, `carrierTowerClockCovariancePolicy`,
`dopplerClockProductCovariancePolicy`, `sharedErrorCovarianceSPD`, `covarianceJitterAdded`,
`nisInterpretation` — all in existing Numerical Summary, no new report chapter.

**(A) Stage 72/73 baselines preserved** — `truthHistoryProductNoisy`, `σ_B=0.01 m`,
compensated carrier slip detection, `syntheticSlipInjection.enable=false`. Navigation
performance and report structure unchanged.

**(B) New config block** — `cfg.covariance.sharedErrors.enable=true`;
`applyTowerClockToCode=true`; `applyTowerClockToCarrier=false`; `applyTowerClockToDoppler=false`;
`carrierPolicy='arcBiasAbsorbsConstantProductBias'`; `dopplerPolicy='simplifiedV1'`.

**(C) Stale metadata removed** — README Stage 68 active block removed; `ReportStatus` updated
to Stage 74; `missingScientificItems(74)` correctly references carrier and Doppler limitations.

NOT ISL. NOT TWSTFT. NOT two-way. NOT multi-asset. No full carrier covariance guarantee.
...
setenv('OO_V1_VALIDATION_STAGE','74')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 74), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles enabled, code block covariance, and compensated slip detection
...
Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 74, default count 4).
