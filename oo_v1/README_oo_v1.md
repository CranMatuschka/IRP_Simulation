...
| Item | Value |
|------|-------|
| Stage | **47** — Guarded Carrier Ionosphere-Free Float EKF Rows v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 47; **full suite not run**) |
...
### Stage 47 validation procedure

Stage 47 adds a guarded carrier ionosphere-free float EKF row path.
`CarrierIonoFreeRowBuilder.buildFromStack` post-processes L1+L2 carrier EKF rows into
IF rows using the alpha/beta combination coefficients.
The IF ambiguity `B_IF = alpha*B_L1 + beta*B_L2` is **not** an integer — float-only,
no integer fixing, no LAMBDA/MLAMBDA, no calibrated DCB/IFB, no PPP-grade claim.
`CarrierIonoFreeEkfDiagnostics` classifies the carrier IF EKF path.
...
setenv('OO_V1_VALIDATION_STAGE','47')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 47), force `test_stage47*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including carrier IF rows, code IF rows, and diagnostics)
4. Verify the PDF: scientific sections present, no "Stage 47 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 47, default count 4).
...
**Current stage:** Stage 47 — Guarded Carrier Ionosphere-Free Float EKF Rows v1

Stage 47 adds a guarded carrier IF EKF row path (float ambiguity only). It does **not** implement
integer ambiguity fixing, LAMBDA/MLAMBDA, calibrated DCB/IFB products, or PPP-grade processing.
The IF ambiguity `B_IF = alpha*B_L1 + beta*B_L2` is not an integer.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 47, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float-only carrier IF — no integer fixing, no wide-lane/narrow-lane, no calibrated DCB, no PPP-grade)
