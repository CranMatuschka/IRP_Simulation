...
| Validation mode | Targeted random smoke (4 selected tests, seed 70; **full suite not run**) |
...
### Stage 70 validation procedure

Stage 70 implements controlled raw-L1 baseline integer ambiguity resolution for the lever-arm
carrier attitude system:

**(A) BaselineCarrierAmbiguityResolver** — new static helper called from `DiffAttitudeBuilder.finalize()`.
For each tower/baseline, computes `N_float = (Σ(ΔΦ−Δρ_ref)) / (n·λ_L1)` using accumulated
sufficient statistics (`accumSum`, `accumSumSq`, `accumN`). Searches integer candidates in
`[round(N_float)−hw, ..., round(N_float)+hw]` (hw=5 cycles). Accepts if
`RMS_residual(N_best) < 0.10·λ_L1` and `ratio = RMS_2nd/RMS_best > 2.0`.
On success: `delta_B = λ_L1 · N_int` and `externalReferenceUsedForCalibration = false`.
On failure: falls back to Stage 69 float `delta_B = accumSum/n`.

**(B) Jacobian correction** — `DiffAttitudeBuilder.buildRows()` now uses
`LinkGeometry.finiteDiffAttitudeJacobian` (quaternion error-state body-frame DCM perturbation)
instead of direct Euler angle perturbation. The differential Jacobian `H = H_att_i − H_att_ref`
correctly represents `∂Δρ/∂δθ` for the QES EKF attitude state.

**(C) Config** — `cfg.estimator.diffAtt.ambiguityResolution.enable=true`;
`useExternalReferenceAsSearchCenter=true` (external attitude positions the float estimate near the
true integer; it is NOT the calibration source when integer fix succeeds).

**(D) Summary fields** — seven fields added to `summary`:
`baselineIntegerFixAttempted`, `baselineIntegerFixAccepted`, `nBaselineIntegerFixed`,
`nBaselineIntegerRejected`, `baselineIntegerFixClassification`,
`externalReferenceUsedAsSearchCenter`, `externalReferenceUsedForCalibration`.

**(E) Report** — "Attitude determination" and "Known limitations" tables in the
"Simulation Physics and Configuration" section updated. No new chapters added.

Stage 69 fixes retained: primary-signal filter, external reference mode, raw L1 DiffAtt,
stochastic clocks, `twoBodyRk4`/`twoBody` dynamics, PCO on, troposphere+ionosphere on.

NOT LAMBDA/MLAMBDA. NOT carrier-IF integer fixing. NOT WL/NL. NOT false-fix-risk control.
NOT operational navigation. NOT PPP-grade. NOT mission-qualified. NOT real-data GNSS processor.
...
setenv('OO_V1_VALIDATION_STAGE','69')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 69), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles enabled and the `singleAssetCarrierAttitude` scenario active
3. Verify the PDF: scientific sections present, Stage 69 in title, Numerical Summary is final section
4. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 69, default count 4).
