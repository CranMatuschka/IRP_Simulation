...
| Item | Value |
|------|-------|
| Stage | **63** — Controlled Single-Asset Integer Ambiguity Fixing v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 63; **full suite not run**) |
...
### Stage 63 validation procedure

Stage 63 adds guarded raw-carrier integer ambiguity fixing for the controlled
`singleAssetCarrierAttitude` scenario. Only `floatPerTowerReceiverSignal` ambiguity states
are eligible. Fixes pass 18 scientific guards and are applied as EKF pseudo-measurements.
Not LAMBDA/MLAMBDA, not carrier-IF fixing, not WL/NL, not false-fix-risk control.

Stage 63:
- adds `IntegerAmbiguityFixer` helper with assess/resetOnSlip/summaryLines
- adds `applyAmbiguityPseudoMeasurement` to `ReverseGNSSEKF` (preserves Stage 62 order)
- integrates fixing into `ReverseGNSSSimulation.step()` after Stage 57 accounting
- tracks held fixes per arc in `fixState63_`; accumulates stats in `fix63Log_`
- adds stage63* summary fields and Stage 63 PDF subsection
- NOT LAMBDA/MLAMBDA, NOT carrier-IF fixing, NOT WL/NL, NOT false-fix-risk control
- NOT PPP-grade attitude claim
...
setenv('OO_V1_VALIDATION_STAGE','63')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 63), force `test_stage63*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled and the `singleAssetCarrierAttitude` scenario active (carrier partials, arc-separated ambiguities, Stage 62 QES covariance, Stage 63 integer fixing)
4. Verify the PDF: scientific sections present, Stage 63 section present
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 63, default count 4).
