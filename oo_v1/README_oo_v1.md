...
| Validation mode | Targeted random smoke (4 selected tests, seed 66; **full suite not run**) |
...
### Stage 66 validation procedure

Stage 66 finalizes the active reverse-GNSS v1 simulation as a single-space-asset, one-way
Earth-tower-to-spacecraft EKF scenario. The multi-asset/ISL/TWSTFT/two-way scaffolding
is removed from the active main-script configuration. Orbit-class configuration is clarified
(`cfg.scenario.orbitClass = 'GEO'`). The final PDF "Single-Asset One-Way Scientific Closure"
section documents topology, measurement model, toggles, limitations, and active metrics.

Stage 66:
- removes GEO-2/ISL/TWSTFT/two-way configuration from `run_oo_reverse_gnss_report.m`; `nSpaceAssets=1` is the direct active default
- adds `cfg.scenario.orbitClass = 'GEO'` with comment that truth is static ECEF in v1
- replaces old "enable then disable later" ISL/TWSTFT pattern with clean absent configuration
- renames final PDF section to "Single-Asset One-Way Scientific Closure" with topology table
- adds `nSpaceAssets`, `orbitClass`, ISL/TWSTFT/two-way disabled flags, and operational-claim=false to PDF table
- fixes stale README item: quaternion error-state EKF exists (Stages 61/62); missing item is production-grade robustness only
- updates ReportStatus, StageHistory, MainScriptValidationGate to Stage 66
- simulation physics, EKF logic, measurement builders, ambiguity logic, and all inactive code paths unchanged
...
setenv('OO_V1_VALIDATION_STAGE','66')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 66), force `test_stage66*` if present (none exists; random selection used)
...
4. Verify the PDF: scientific sections present, Stage 66 in title
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 66, default count 4).
...
**Current stage:** Stage 66 — Single-Asset One-Way Realistic Reverse-GNSS v1 Closure

Stage 66 finalizes the active scenario as a clean single-space-asset, one-way Earth-tower-to-spacecraft
EKF simulation. ISL/TWSTFT/two-way/multi-asset scaffolding removed from active configuration.
Orbit-class field added. PDF closure section documents topology, measurement model, and limitations.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 66, default count 4). Full suite is **NOT RUN**.
