...
| Item | Value |
|------|-------|
| Stage | **44** — Dual-Frequency IF Consistency and Bias Budget v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 44; **full suite not run**) |
...
### Stage 44 validation procedure

Stage 44 adds `SignalConfigResolver` for consistent L2 detection from any cfg field, fixes the
`l2SignalEnabled_` bug in `IonosphereFreeCombinationDiagnostics`, and adds `IonosphereFreeBiasBudget`
for diagnostic IF bias residual propagation. This is **diagnostic only**: IF rows are not in the
EKF, no integer ambiguity fixing, no LAMBDA/MLAMBDA, no PPP-grade claim. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','44')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 44), force `test_stage44*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including IF bias budget diagnostic)
4. Verify the PDF: scientific sections present, no "Stage 44 Validation Status" heading
...
**Current stage:** Stage 44 — Dual-Frequency IF Consistency and Bias Budget v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 44, default count 4). Full suite is **NOT RUN**.
