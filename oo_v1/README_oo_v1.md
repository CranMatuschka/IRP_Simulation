...
| Item | Value |
|------|-------|
| Stage | **48** — Carrier Ionosphere-Free Ambiguity Traceability v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 48; **full suite not run**) |
...
### Stage 48 validation procedure

Stage 48 adds explicit traceability for the L1/L2 ambiguity state pair behind each
carrier ionosphere-free (IF) EKF row.
`CarrierIonoFreeRowBuilder.buildFromStack` now stores `ambiguityStateIdxL1`,
`ambiguityStateIdxL2`, `ambiguityStateIdxPair`, and `ambiguityWeights` in `cpInfo`.
`CarrierIonoFreeAmbiguityTraceability` propagates the IF ambiguity variance
`Var(B_IF) = [alpha beta] * P_pair * [alpha; beta]'` from the Stage 41 covariance export.
The IF ambiguity `B_IF = alpha*B_L1 + beta*B_L2` is **not** an integer — traceability only,
no integer fixing, no LAMBDA/MLAMBDA, no calibrated DCB/IFB.
...
setenv('OO_V1_VALIDATION_STAGE','48')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 48), force `test_stage48*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including carrier IF rows, traceability, code IF rows, and diagnostics)
4. Verify the PDF: scientific sections present, no "Stage 48 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 48, default count 4).
...
**Current stage:** Stage 48 — Carrier Ionosphere-Free Ambiguity Traceability v1

Stage 48 adds explicit traceability for the L1/L2 ambiguity state pair behind each carrier
ionosphere-free (IF) EKF row. The ionosphere-free combination `B_IF = alpha*B_L1 + beta*B_L2`
is not an integer. Var(B_IF) is propagated from the Stage 41 covariance export for diagnostics
only. Integer fixing, LAMBDA/MLAMBDA, calibrated DCB/IFB products are NOT implemented in v1.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 48, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float-only carrier IF rows; Stage 48 adds float traceability — no integer fixing, no wide-lane/narrow-lane, no calibrated DCB, no PPP-grade)
