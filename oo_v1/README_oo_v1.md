...
| Item | Value |
|------|-------|
| Stage | **59** — Single-Space-Asset Multi-Antenna Carrier Attitude Scenario v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 59; **full suite not run**) |
...
### Stage 59 validation procedure

Stage 59 adds a controlled single-space-asset multi-antenna float-carrier attitude scenario.
One estimated GEO asset with 4 receiver antennas in a non-collinear cross-pattern body-frame
geometry (lever-arm baseline rank 3). Carrier attitude partials are enabled via the preferred
Stage 56 `cfg.estimator.attitude.useCarrierPartials` control. EKF float ambiguities use
arc-separated ambiguity metadata (Stage 53) with enforced carrier arc consistency (Stage 54).
EKF dynamics are set to `constantVelocity` for self-consistency with the static-ECEF GEO truth
(no truth orbit propagator in v1). ISL/TWSTFT rows are disabled. The scenario reports attitude
error norm, carrier residual RMS, physical NIS-per-DOF, baseline rank, and dynamics
self-consistency. No integer fixing, LAMBDA/MLAMBDA, calibrated phase-bias products,
PPP-grade attitude claims, or multi-space-asset estimation. Simulation duration: 3600 s.
...
- no multi-space-asset estimation
...
setenv('OO_V1_VALIDATION_STAGE','59')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 59), force `test_stage59*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled and the `singleAssetCarrierAttitude` scenario active (ISL disabled, CV dynamics, 4-receiver cross-pattern, carrier attitude partials, arc consistency enforcement, Stage 57 physical/gauge NIS accounting)
4. Verify the PDF: scientific sections present, no "Stage 59 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 59, default count 4).
...
**Current stage:** Stage 59 — Single-Space-Asset Multi-Antenna Carrier Attitude Scenario v1

Stage 59 adds the canonical single-space-asset multi-antenna carrier attitude scenario.
It is float carrier attitude only. No integer fixing, no LAMBDA/MLAMBDA, no false-fix-risk
control, no calibrated phase-bias products, no PPP-grade attitude claim, no multi-space-asset
estimation. Full suite is **NOT RUN**.
