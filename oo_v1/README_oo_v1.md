...
| Item | Value |
|------|-------|
| Stage | **46** — Code IF EKF Consistency and Traceability v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 46; **full suite not run**) |
...
### Stage 46 validation procedure

Stage 46 adds `CodeIonoFreeConsistencyDiagnostics` to audit the Stage 45 code IF EKF path:
row-count traceability, H Jacobian compatibility assumptions, R/noise amplification,
residual/NIS availability, and signal-dependent bias-state risk. Also adds `combineJacobians`
to `CodeIonoFreeRowBuilder` for synthetic H-combination tests and future row-level checks.
This is a **consistency audit only**: no carrier IF rows, no integer ambiguity fixing,
no LAMBDA/MLAMBDA, no calibrated DCB/IFB, no PPP-grade claim. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','46')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 46), force `test_stage46*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including code IF EKF rows and consistency diagnostic)
4. Verify the PDF: scientific sections present, no "Stage 46 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 46, default count 4).
...
**Current stage:** Stage 46 — Code IF EKF Consistency and Traceability v1

Stage 46 is a consistency audit for the Stage 45 code IF EKF path. It does **not** implement
carrier IF EKF rows, calibrated DCB/IFB products, integer ambiguity fixing, LAMBDA/MLAMBDA,
higher-order ionosphere modelling, or PPP-grade processing.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 46, default count 4). Full suite is **NOT RUN**.
...
- EKF use of ionosphere-free carrier rows (carrier IF, P3-like carrier; Stage 45 adds code IF EKF rows; Stage 46 audits the code IF path — no carrier IF, no integer fixing, no LAMBDA/MLAMBDA, no higher-order iono, no PPP-grade)
