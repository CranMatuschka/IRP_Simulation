...
| Item | Value |
|------|-------|
| Stage | **27-28** — Validation Artifact Closure + Orbit Dynamics Integration Diagnostics |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 28; **full suite not run**) |
| Authoritative runtime metadata | Generated by `ReportStatus` and stored in `output/latest_validation_summary.json` |
| Commit SHA | See `output/latest_validation_summary.json` (changes on each commit; not hard-coded here) |
| MATLAB version | R2025b |

> **Note:** Stages 27-28 deliberately do not run the full test suite.
> The authoritative commit SHA and test pass/fail count are written to
> `output/latest_validation_summary.json` at runtime by `run_stage28_validation.m`.
> Do not hard-code a commit SHA in this README — it becomes stale on the next commit.

### Stage 28 validation procedure

```matlab
cd oo_v1
run_stage28_validation   % selects 4 tests (seed 28, forces stage28 test), runs main script, verifies PDF
```
...
- `output/notion_stage28_update.md` — ready-to-paste Notion update block

Stage 27 gate (subset of Stage 28; useful if Stage 28 is broken):
```matlab
run_stage27_validation   % selects 4 tests (seed 27, forces stage27/26 test)
```

### All-toggle report run

To run the main report with all independent boolean toggles enabled manually:

```matlab
% Via env var (Stage 25 gate):
setenv('OO_V1_ALL_TOGGLES', 'true');
run_oo_reverse_gnss_report
setenv('OO_V1_ALL_TOGGLES', '');

% Or directly in run_oo_reverse_gnss_report.m, set:
stageAllToggles = true;
run_oo_reverse_gnss_report
```
