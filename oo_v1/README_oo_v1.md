...
| Item | Value |
|------|-------|
| Stage | **71** — Realistic Clock Product and Clock-State Handling |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 71; **full suite not run**) |
...
### Stage 71 validation procedure

Stage 71 replaces the active truth-plus-noise tower clock correction with a realistic synthetic
broadcast-product-like clock correction model:

**(A) Product-prediction mode** — `cfg.errors.towerClockCorrection.mode = 'truthHistoryProductNoisy'`.
Product epoch is `t_prod = floor((t − latency) / dT) * dT` (quantised, delayed). At `t_prod`,
truth clock bias and drift are read from tower history. A deterministic per-`(tower, t_prod)`
noise draw (seeded persistent cache in `TowerClockCorrectionProvider`) adds product bias and
drift error fixed at broadcast time. Prediction to measurement time: `b_model = (b_true + b_noise)
+ (bdot_true + d_noise) * age`. Correction sigma: `sqrt(σ_B² + age² * σ_D²)` added to code
and carrier measurement covariance diagonal. Doppler clock-drift sigma simplified v1 (not added
to Doppler R, reported in summary).

**(B) Tower vs receiver clock** — receiver clock bias and drift remain estimated by the EKF
(Brown-Hwang two-state process model). Tower clock states are NOT estimated (`estimateTowerClocks=false`)
to avoid clock gauge ambiguity. Tower clocks are corrected by product only.

**(C) Product config** — `updateInterval=30 s`, `latency=5 s`, `σ_B=0.05 m (~0.15 ns)`,
`σ_D=0.001 m/s`. Max product age = 35 s; mean product age = 20 s.
Max product sigma ≈ 0.057 m; mean product sigma ≈ 0.051 m. Shared tower-product covariance
is flagged in summary; diagonal R-only implementation in v1.
Code measurement R inflated by product sigma; carrier R NOT inflated (float ambiguity absorbs clock bias).

**(D) Carrier R inflation** — `CarrierMeasurementBuilder.buildEkfRows()` now accepts and applies
`towerClkSigma` per carrier row (previously the parameter was silently discarded with `~`).

**(E) Summary fields** — eleven Stage 71 fields added:
`towerClockProductMode`, `towerClockPerfectCorrection`, `towerClockProductUpdateInterval_s`,
`towerClockProductLatency_s`, `towerClockProductMeanAge_s`, `towerClockProductMaxAge_s`,
`towerClockProductMeanSigma_m`, `towerClockProductMaxSigma_m`, `towerClockSharedCovarianceApplied`,
`receiverClockEstimated`, `towerClockStatesEstimated`, `dopplerClockProductUncertaintyStatus`.

**(F) Allan deviation** — preserved from Stage 67. One log-log ADEV figure for asset receiver
clock and each tower truth clock. It is computed from simulated truth clock histories, not hardware.

Stage 70 integer fix and Stage 69 attitude fixes fully retained.

NOT real CLK product ingestion. NOT operational timing. NOT PPP-grade. NOT mission-qualified.
...
setenv('OO_V1_VALIDATION_STAGE','71')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 71), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles enabled and the `singleAssetCarrierAttitude` scenario active
3. Verify the PDF: scientific sections present, Stage 71 in title, Numerical Summary is final section
4. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 71, default count 4).
