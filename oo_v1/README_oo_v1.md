...
| Item | Value |
|------|-------|
| Stage | **72** — Clock Product Consistency and Navigation Recovery |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 72; **full suite not run**) |
...
### Stage 72 validation procedure

Stage 72 is a repair stage. It fixes the Stage 71 navigation regression (27.9 m position error)
and corrects stale report text.

**Root cause (Stage 71 regression — carrier phase false slip detection):**
Each 30 s product-epoch boundary causes a step in the tower clock model correction equal to
`(b_noise_new − b_noise_old) ~ N(0, sqrt(2)·σ_B)`. `CycleSlipDetector` compares consecutive
pre-fit carrier residuals and flags `|Δresidual| ≥ threshold_m = 0.1 m` as a cycle slip. With
`σ_B = 0.05 m` the inter-epoch jump RMS was 0.07 m, giving a **~15 % false-slip rate** per
measurement per epoch change. With 5 towers × 4 receivers = 20 carrier measurements and 120
product-epoch changes in 3600 s, roughly 360 false ambiguity resets occurred, preventing carrier
phase convergence and leaving position accuracy at code-only level (~28 m).

Note: the `clockAtProductEpoch` startup-history bug (history empty at `k=1`, `t_prod=0`
predating recorded history) was also present but contributed < 1 mm to the innovation because
tower clocks initialise at `(bias=0, drift=0)`.

**Fixes:**
1. `sigmaBias_m` reduced from 0.05 → **0.01 m** (0.033 ns, IGS-class product accuracy). The
   inter-epoch jump sigma is now 0.014 m — **7 σ below the 0.1 m slip threshold** — giving a
   false-slip probability of ~10⁻¹² per measurement. Carrier phase ambiguities now converge normally.
2. `GroundTower` constructor seeds history with the `t=0` initial clock state so
   `clockAtProductEpoch(t_prod=0)` always finds a valid entry.
3. `clockAtProductEpoch` "predates history" fallback corrected: previously used the earliest history
   entry's values but still applied age `= t_s − t_prod`, projecting from the wrong time anchor;
   now uses current tower state (zero-age anchor, safer when b(0) ≈ 0).

**(A) Product-prediction mode** — `cfg.errors.towerClockCorrection.mode = 'truthHistoryProductNoisy'`.
Active tower clock correction is synthetic product-predicted, not perfect and not truth-plus-noise.
Product epoch delayed/quantised; per-product deterministic noise; linear prediction to measurement time.

**(B) Tower vs receiver clock** — receiver clock estimated; tower clock states not estimated
(`estimateTowerClocks=false`) to avoid gauge ambiguity.

**(C) Product config** — `updateInterval=30 s`, `latency=5 s`, **`σ_B=0.01 m (~0.033 ns)`**,
`σ_D=0.0002 m/s`. Max product age = 35 s; mean product age = 20 s.
Max product sigma ≈ 0.014 m; mean product sigma ≈ 0.010 m.
Covariance handling: `towerClockSharedCovarianceApplied=false`, `towerClockProductDiagonalInflation=true`
(diagonal per-row code-R inflation only; no off-diagonal shared-R in v1).
Carrier R NOT inflated (float ambiguity absorbs clock bias).

**(D) Report fixes** — summary fields moved before PDF generation (NaN metadata fixed);
gauge text updated from stale "perfect truth subtracted" to product-corrected description;
covariance status now reports diagonal-inflation-only honestly.

**(E) Recovery criteria met:** final 3D position error < 10 m; final receiver clock error
magnitude < 10 m; product metadata finite (no NaN); attitude errors < 2 deg; KAV converged.

NOT real CLK product ingestion. NOT ISL. NOT TWSTFT. NOT two-way. NOT multi-asset.
...
setenv('OO_V1_VALIDATION_STAGE','72')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 72), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles enabled
...
Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 72, default count 4).
...
**Current stage:** Stage 68 — Single-Asset One-Way Realistic Physics Closure

Stage 68 completes the single-asset one-way scenario physics:
(A) PDF restructured: stage-titled science chapters removed; "Simulation Physics and Configuration"
section (6 compact tables) now precedes "Numerical Summary" as the final section;
(B) antenna PCO enabled by default (synthetic; no ANTEX);
(C) troposphere + ionosphere documented as always-on defaults.
Stage 67 upgrades retained: stochastic clocks, matched two-body dynamics, quaternion EKF attitude.
...
setenv('OO_V1_VALIDATE_REPORT','true')
setenv('OO_V1_ALL_TOGGLES','true')
setenv('OO_V1_VALIDATION_STAGE','68')
run_oo_reverse_gnss_report
```

**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 68, default count 4). Full suite is **NOT RUN**.
