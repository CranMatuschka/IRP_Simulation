...
| Item | Value |
|------|-------|
| Stage | **67** — Single-Asset Attitude, Clock, and Dynamics Realism Closure |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 67; **full suite not run**) |
...
### Stage 67 validation procedure

Stage 67 makes three physical realism upgrades to the Stage 66 single-asset one-way simulation:

**(A) Attitude** — primary estimator labelled as carrier lever-arm quaternion error-state EKF (Stages 61/62).
`coarseBaselineIntegerSearch` is the optional initializer only; `calibratedDifferentialAmbiguity` provides
relative tracking only. `cfg.estimator.attitude.primaryMode = 'carrierLeverArmQuaternionEkf'` added.

**(B) Clocks** — stochastic receiver and tower clocks replace perfect corrections.
`cfg.clock.receiver.deterministic = false`; `cfg.errors.towerClockCorrection.mode = 'noisyCorrection'`;
tower clock `deterministic = false` set per-tower in `ScenarioPresets`. Overlapping Allan deviation
plot added to PDF Oscillator Stability section (`AllanDeviation` class, `+revgnss/AllanDeviation.m`).

**(C) Dynamics** — matched twoBodyRk4 truth propagator + twoBody EKF dynamics replace static-ECEF truth.
`ScenarioPresets.singleAssetCarrierAttitude` sets `orbit.useOrbitPropagator = true`, `orbit.mode = 'twoBodyRk4'`,
`estimator.dynamics.mode = 'twoBody'` (GEO, 35786 km, equatorial, ν₀ = 23°).

Stage 67 also adds a "Stage 67 Attitude, Clock, and Dynamics Realism Closure" section to the PDF
with compact tables for attitude, clock, and dynamics, plus Allan deviation plot reference.
...
setenv('OO_V1_VALIDATION_STAGE','67')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 67), force `test_stage67*` if present (none exists; random selection used)
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled and the `singleAssetCarrierAttitude` scenario active
4. Verify the PDF: scientific sections present, Stage 67 in title
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 67, default count 4).
...
**Current stage:** Stage 67 — Single-Asset Attitude, Clock, and Dynamics Realism Closure

Stage 67 upgrades the Stage 66 single-asset one-way scenario with three physical realism improvements:
(A) attitude estimator clarified as carrier lever-arm quaternion error-state EKF;
(B) stochastic tower and receiver clocks with Allan deviation plot;
(C) matched twoBodyRk4 truth propagator + twoBody EKF dynamics (GEO equatorial, 35786 km).
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 67, default count 4). Full suite is **NOT RUN**.
