...
| Item | Value |
|------|-------|
| Stage | **69** — Carrier-Lever-Arm Attitude Closure |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 69; **full suite not run**) |
...
### Stage 69 validation procedure

Stage 69 fixes the active carrier-lever-arm attitude solution:

**(A) Primary-signal filter** — `DiffAttitudeBuilder.accumulate()` and `buildRows()` now select only the
lowest `signalIdx` row per tower-antenna pair. With `twoFrequency.enable=true` producing two rows
(L1+L2), the previous code produced `sum(mask)!=1` and silently skipped every pair, resulting in
zero calibrated baselines. The primary (L1) signal is now consistently selected.

**(B) External calibration reference** — `referenceMode='externalInitialAttitude'` causes
`DiffAttitudeBuilder` to evaluate the geometric model at the reference attitude (truth + 0.1 deg noise)
rather than the current EKF attitude during the calibration window. This prevents `delta_B` from
absorbing the initial EKF attitude error (~1 deg), which previously made all post-calibration
innovations near-zero and blocked attitude convergence.

**(C) Raw L1 for DiffAtt** — `ReverseGNSSSimulation.step()` switches `cpDA` to `floatRows` (raw L1)
when IF combination is active, so DiffAtt always operates on raw L1 carrier phases rather than
the IF-combined signal.

**(D) Coarse initializer disabled** — `attitudeInitMode='none'` prevents a potentially wrong integer
search from injecting a bad initial attitude into the quaternion EKF. The
`externalInitialAttitude` reference is now the sole absolute attitude anchor.

**(E) Position plot X/Y/Z** — `plotPositionError_()` now shows X/Y/Z ECEF components + 3D norm,
using the existing `Diagnostics.getPositionErrorVecs()` method.

Stage 68 physics retained: stochastic clocks, matched `twoBodyRk4`/`twoBody` dynamics, PCO on,
troposphere+ionosphere on, "Simulation Physics and Configuration" section in PDF.
