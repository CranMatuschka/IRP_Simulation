...
| Item | Value |
|------|-------|
| Stage | **51** — Ambiguity Readiness Evidence Hardening v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 51; **full suite not run**) |
...
### Stage 51 validation procedure

Stage 51 hardens the evidence behind the Stage 50 ambiguity fixing readiness gate.
`AmbiguityFixingReadinessGate` is updated to collect all evidence without early return,
expose `arcQuality` and `residualConsistency` as public diagnostics, and provide a
`blockerList` and `readinessScore`. Arc-quality checks whether actual slip/arc count
summaries exist (not just whether detection is enabled). This is still a **readiness
gate only** — no ambiguity is fixed, rounded, or resolved. Integer fixing, LAMBDA/MLAMBDA,
...
setenv('OO_V1_VALIDATION_STAGE','51')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 51), force `test_stage51*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including evidence hardening, readiness gate, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 51 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 51, default count 4).
...
**Current stage:** Stage 51 — Ambiguity Readiness Evidence Hardening v1

Stage 51 hardens the evidence behind the Stage 50 readiness gate. Evidence is now
collected without early return (all available fields are reported even when prerequisites
are missing). Arc quality checks for actual slip/arc count summaries. Residual/NIS
consistency checks for finite NIS or prefit RMS. This is still a readiness gate only —
no ambiguity is fixed or rounded. Integer fixing, LAMBDA/MLAMBDA, calibrated phase-bias
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float IF rows; Stage 48 adds float traceability; Stage 49 adds float WL/NL diagnostics; Stage 50 adds readiness gate; Stage 51 hardens readiness evidence — no integer fixing, no calibrated phase-bias products, no PPP-grade)
