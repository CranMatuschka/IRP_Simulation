...
| Item | Value |
|------|-------|
| Stage | **73** — Carrier Arc and Cycle-Slip Robustness |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 73; **full suite not run**) |
...
### Stage 73 validation procedure

Stage 73 makes carrier slip detection product/model-step aware. Known tower-clock product boundary
steps are compensated before slip testing. DiffAtt baseline slip handling is baseline-specific.
Ambiguity resets are local to affected arcs. Product boundary resets are zero in nominal validation.
Full suite NOT RUN. No operational cycle-slip guarantee; v1 synthetic robustness only.

**Scientific fix (Stage 73 — model-step-compensated carrier slip detection):**
Stage 72 fixed the false-slip problem by reducing `σ_B` so product epoch boundary jumps rarely
exceeded the slip threshold. Stage 73 addresses this more robustly at the algorithm level:
the slip detector now subtracts the **known model correction step** before applying the threshold.
...
where `observedJump = prefit_k − prefit_{k-1}` (signed) and
`expectedModelJump = towerClkModel_k − towerClkModel_{k-1}` (signed, per-track).

Tower clock model values (`cpInfo.towerClkModel_m`) are passed through from
`CarrierMeasurementBuilder.buildEkfRows()` and stored per-track in `CarrierTrackManager`.
At each product epoch boundary the expected jump is computed and subtracted; the residual
`slipMetric` reflects only real phase discontinuities. A real cycle slip (B_true changes by
Δ cycles) produces `slipMetric ≈ Δ·λ ≈ 0.19 m >> 0.10 m` regardless of product update.

**Why this is more robust than lowering σ_B alone:**
- Works for any σ_B value (immune to product quality choice)
- Sensitive to real slips; immune to known model steps
- Does not require σ_B < threshold/(7√2) to avoid false slips
- Enables future testing with lower-quality products (larger σ_B) without retuning

**DiffAtt baseline handling:**
Differential carrier cancels the common tower clock term between receiver pairs. DiffAtt slip
detection was already disabled per Stage 69 design (innovation gate ≡ the only guard). No change
to DiffAtt logic needed; 15/15 baselines remain active in nominal run.

**Diagnostics fields added:**
`nCarrierProductBoundaries`, `nCarrierProductBoundariesCompensated`, `nConfirmedCarrierSlips`,
`nFalseProductBoundaryResets`, `nUnclassifiedCarrierJumps`, `nDiffAttBaselineResets`,
`nDiffAttBaselinesActiveFinal`, `carrierSlipDetectorMethod`, `productStepCompensationEnabled`,
`syntheticSlipInjectionEnabled`, `carrierArcRobustnessStatus` — all in existing Numerical Summary.

**(A) Stage 72 baseline preserved** — `truthHistoryProductNoisy`, `σ_B=0.01 m`, `σ_D=0.0002 m/s`,
`updateInterval=30 s`, `latency=5 s`. Navigation performance unchanged.

**(B) New config block** — `cfg.carrierSlip.method = 'modelStepCompensatedResidualJump'`;
`cfg.carrierSlip.productStepCompensation = true`; `syntheticSlipInjection.enable = false` (final run).

**(C) Nominal run criteria:** product boundaries counted and compensated; `nConfirmedCarrierSlips=0`;
`nFalseProductBoundaryResets=0`; `nDiffAttBaselineResets=0`; DiffAtt baselines 15/15 active;
final position error < 10 m; clock metric < 10 m; KAV < 2 deg; report structure unchanged.

NOT ISL. NOT TWSTFT. NOT two-way. NOT multi-asset. No synthetic cycle-slip guarantee.
...
setenv('OO_V1_VALIDATION_STAGE','73')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 73), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles enabled and compensated slip detection
...
Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 73, default count 4).
