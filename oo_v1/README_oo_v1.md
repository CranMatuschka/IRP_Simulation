...
| Item | Value |
|------|-------|
| Stage | **68** — Single-Asset One-Way Realistic Physics Closure |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 68; **full suite not run**) |
...
### Stage 68 validation procedure

Stage 68 completes the active single-asset one-way simulation physics:

**(A) PDF restructure** — stage-titled science chapters removed from report body. A non-stage-titled
"Simulation Physics and Configuration" section (6 compact tables: topology, measurement model,
atmosphere/antenna/bias, attitude, clocks, orbital dynamics) is inserted before "Numerical Summary",
which is now the final report section.

**(B) Antenna PCO** — `cfg.effects.antennaPCO.truth.enable` and `.model.enable` set to `true` by default
(synthetic calibrated constants; no ANTEX). PCV remains disabled by default.

**(C) Atmosphere active defaults** — troposphere (`simpleMapped`, Saastamoinen-style, truth+model on)
and ionosphere (`simpleMapped`, 1/f² dispersion, truth+model on) documented as always-on defaults.
No GPT3/VMF3/ERA5 troposphere; no Klobuchar/IONEX ionosphere.

Stage 67 upgrades retained: stochastic clocks, matched `twoBodyRk4`/`twoBody` dynamics, quaternion EKF
attitude labelling, Allan deviation plot in Oscillator Stability section.
...
This will:
1. Select 4 random tests (seed 68), stop before report generation if any selected test fails
2. Run the main 3600 s report with all-toggles enabled and the `singleAssetCarrierAttitude` scenario active
3. Verify the PDF: scientific sections present, Stage 68 in title, Numerical Summary is final section
4. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 68, default count 4).
