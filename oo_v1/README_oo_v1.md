...
| Item | Value |
|------|-------|
| Stage | **75** — Baseline Attitude Ambiguity Classification and False-Fix Hardening |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 75; **full suite not run**) |
...
### Stage 75 validation procedure

Stage 75 hardens baseline attitude ambiguity classification with per-baseline status, tightened
gates, a float-distance guard, and an honest GNSS-only claim policy. Stages 73 (slip detection)
and 74 (block covariance) are fully preserved. Full suite NOT RUN. Single-asset one-way setup unchanged.

**Scientific additions (Stage 75 — ambiguity classification hardening):**

*Per-baseline `ambiguityStatus`* — each `(tower, baseline)` pair gets one of:
`fixedInteger`, `rejectedInsufficientArc`, `rejectedRms`, `rejectedRatio`,
`rejectedFloatDistance`, `floatExternalReference`.

*New float-distance gate* — `|N_float - N_best| < 0.25 cycles`. Rejects candidates where the
integer best-match is implausibly far from the float estimate, even if RMS and ratio tests pass.

*Tightened gates* — `ratioThreshold` raised from 2.0 to 3.0; `minArcEpochs` raised from 10 to 60.
These values are scientifically conservative for a 3600 s controlled-synthetic run.

*Partial-fix policy* — `partialFixPolicy = 'useFixedOnlyOrExplicitMixed'`: when only some baselines
are fixed, non-fixed baselines are excluded from EKF rows (not silently kept as float).

*Honest GNSS-only claim* — `gnssOnlyAttitudeClaim = true` ONLY when ALL active baselines are
`fixedInteger` AND no external-reference calibration was used.
`requireAllForGnssOnlyClaim = true` enforces the stricter requirement.

*Phase-bias and false-fix documentation* — `phaseBiasStatus = 'notCalibratedExternalProduct'`
(differential phase bias not independently calibrated); `falseFixClassification = 'screenedNotFormal'`
(no LAMBDA/ILS; only RMS/ratio/float-distance screening).

**Summary fields added (Stage 75):**
`baselineArClassification`, `baselineArGnssOnlyClaim`, `baselineArFalseFixClassification`,
`baselineArPhaseBiasStatus`, `baselineArPartialPolicy`, `nBaselineArFixed`,
`nBaselineArRejectedArc`, `nBaselineArFloatExternal`, `nBaselineArUsedInEkf`,
`externalRefUsedForAnyCalibration` — all in existing Numerical Summary/Physics tables, no new chapter.

**(A) Stage 72/73/74 baselines preserved** — `truthHistoryProductNoisy`, `σ_B=0.01 m`,
compensated carrier slip detection, block covariance for code rows. Navigation performance
and report structure unchanged.

NOT ISL. NOT TWSTFT. NOT two-way. NOT multi-asset.
No LAMBDA/MLAMBDA. No calibrated phase-bias products. No formal ILS false-fix-risk control.
...
setenv('OO_V1_VALIDATION_STAGE','75')
