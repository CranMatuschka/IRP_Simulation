...
| Item | Value |
|------|-------|
| Stage | **34** — Attitude Jacobian Consistency Audit v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 34; **full suite not run**) |
...
### Stage 34 validation procedure

Stage 34 adds an attitude Jacobian consistency audit. In production the audit runs in
H-only summary mode (row-type metadata only; no per-row LOS vectors). Finite-difference
consistency is available when per-row structs with `los_ref_unit`, `lever_body_m`, and
`rpy_rad` fields are supplied (test harness or future integration). No quaternion EKF,
no attitude accuracy claim, no integer ambiguity fixing.
...
setenv('OO_V1_VALIDATION_STAGE','34')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 34), force `test_stage34*` if present
2. Stop before report generation if any selected test fails
3. Run the main report with all-toggles enabled
