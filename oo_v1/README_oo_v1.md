...
| Item | Value |
|------|-------|
| Stage | **85** — Formal Synthetic Validation Campaign and Statistical Consistency Audit |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Main-script validation gate (4 selected tests, seed 85; **full suite not run**) + light campaign (4 cases × 3 seeds × 900 s) |
...
### Stage 85 validation procedure

Stage 85 adds a formal synthetic validation campaign inside the single main-script invocation.

**New infrastructure (Stage 85):**

*`ScientificValidationCampaign`* — runs configurable stress cases (nominalDualFrequency, l1Only,
degradedClockProduct, slipInjection) over multiple seeds and returns aggregated performance/consistency
statistics. All results labelled `partialCovarianceAwareSynthetic` — not real-world proof.

*`ConsistencyStatistics`* — classifies per-group NIS (code/carrier/Doppler) and NEES (pos/vel/clk/att)
from completed `Diagnostics` objects. Status: `pass` if mean NIS/DOF ∈ [0.5, 2.0]; `warnHigh`/`warnLow`
outside that range. Explicitly caveated as synthetic consistency evidence only.

*`StressScenarioFactory`* — config mutations: `l1Only` (disable L2), `degradedClock` (scale product σ × 3),
`slipInjection` (add synthetic cycle slips via `cfg.validation.stress.slips`), `reducedGeometry` (4 towers).

*`Diagnostics` additions* — NEES_vel, NEES_clk, NEES_att per epoch; numDopplerRows; getter methods
`getVelocityNEES()`, `getClockNEES()`, `getAttitudeNEES()`.

*`CarrierMeasurementBuilder`* — synthetic slip injection hook reads `cfg.validation.stress.slips.*`.

*`ConfigFactory` Stage 85 block* — owns all `cfg.validation.scientificCampaign.*`,
`cfg.validation.statistics.*`, and `cfg.validation.stress.*` defaults.

*`ModelCoverageAudit`* — category 21 (`validationStatistics`) reports `implementedSynthetic` when
campaign is enabled; `disabledByConfig` otherwise.

**To run Stage 85 validation:**

```matlab
setenv('OO_V1_VALIDATE_REPORT','true');
setenv('OO_V1_ALL_TOGGLES','true');
setenv('OO_V1_VALIDATION_STAGE','85');
run_oo_reverse_gnss_report
```

Expected: pos < 10 m, clock < 10 m, KAV < 2 deg, 4/4 tests pass, PDF generated.
Campaign (12 extra 900 s runs): `campaignOverallStatus = pass`.
NIS statuses: `pass` or `insufficientSamples`; NEES statuses: `pass` or `warnHigh` (P too small = honest).
...
### Stage 84 validation procedure (archived)

Stage 84 hardens Doppler R covariance correctness and product-epoch drift semantics.

**Core fixes (Stage 84):**

*Doppler R diagonal double-count* — `DopplerMeasurementBuilder` previously added `σ_drift²` to the
diagonal before calling `addDopplerDriftBlock`, which also adds the full `σ_drift²·ones(n,n)` block
(diagonal included). Result was diagonal = `σ_dop² + 2·σ_drift²`. Fix: build `Rd` as tracking-only
(`σ_dop²·I`) and let the helper own all drift covariance (diagonal + off-diagonal). New diagnostic:
`dopplerDriftVarianceDiagonalPolicy = trackingOnlyPlusBlock`.

*`computeDrift()` product-epoch anchor* — For `truthHistoryProductNoisy` mode,
`TowerClockCorrectionProvider.computeDrift()` now reads `clockAtProductEpoch()` for the truth drift
anchor instead of the current-epoch truth drift. Consistent with the bias path in `compute()`.
New metadata: `driftAnchorStatus`, `driftProductMode`, `driftSigmaSource`, `explicitProductDriftUsed`,
`truthHistoryProductDriftUsed`.

*`ModelCoverageAudit` covariance gate* — category 20 now uses `cfg.covariance.productClock.enable`
instead of the legacy `cfg.covariance.sharedErrors.enable`; reports `codeProductCovarianceStatus`,
`dopplerProductCovarianceStatus`, `carrierProductCovarianceStatus` flags.

*J2 ratio diagnostics* — `ConfigFactory` Stage 84 block computes `sigmaToRmsJ2Ratio` and
`sigmaToMaxJ2Ratio`; wording corrected from "fully absorbed" to "tolerated by configured process noise".

*Carrier arc reference status* — `CarrierMeasurementBuilder` reports
`carrierProductArcReferenceStatus = notAvailableUsingProductEpochAgeV1`.

*Carrier-Doppler consistency* — explicitly guarded: `carrierDopplerConsistencyStatus = notImplementedGuarded`.
...
setenv('OO_V1_VALIDATE_REPORT', 'true')
setenv('OO_V1_ALL_TOGGLES',     'true')
setenv('OO_V1_VALIDATION_STAGE','84')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 tests (seed 84) from the active-scope pool, stop if any fail
2. Run the full 3600 s simulation with all features enabled
3. Verify gates: pos RMS < 10 m, clock RMS < 10 m, KAV < 2 deg
4. Generate the PDF report
5. Mark the validation artifact with stage 84 and runtime SHA

### Stage 83 validation procedure (archived)

Stage 83 adds frame-consistent Doppler (ECI-consistent range-rate with tower rotational velocity,
product-drift model, and product-drift covariance blocks) and time-varying carrier product residual
covariance (age-weighted outer product; constant bias absorbed by float ambiguity).

**Doppler dynamics additions (Stage 83):**

*Tower ECI rotational velocity* — `OneWayRangeRateModel` adds `sagnacRate = ω_e(u_y·Δx − u_x·Δy)`
to both truth and model Doppler; equivalent to `u'·(v_rx_ecef + ω×Δ)`. Contribution ≈ 0.1–0.8 m/s
for GEO geometry. `sagnacRateHandling = capturedByTowerVelocityTerm`; no double-count with iterative
light-time.

*Product drift in Doppler* — `TowerClockCorrectionProvider.computeDrift()` provides consistent
product-epoch clock drift to both truth and model; previously zeroed for non-perfectCorrection modes.

*Doppler R covariance blocks* — `ProductClockCovarianceBuilder.addDopplerDriftBlock()` adds
`σ_drift²·ones(n,n)` for rows sharing the same (tower, product epoch). SPD guard via jitter_m2=1e-12.

*Carrier product residual covariance* — `ProductClockCovarianceBuilder.addCarrierDriftBlock()` adds
`age_i·age_j·σ_drift²` blocks. Policy: `timeVaryingProductResidualOnly`.
...
setenv('OO_V1_VALIDATE_REPORT', 'true')
setenv('OO_V1_ALL_TOGGLES',     'true')
setenv('OO_V1_VALIDATION_STAGE','83')
run_oo_reverse_gnss_report
```
...
(J2-perturbed RK4) with `twoBody` EKF predictor. A representative J2 acceleration at the initial
GEO orbit state is computed and recorded; the existing `sigma_accel=0.01 m/s²` already satisfies
the `>= 0.1 × a_J2` process-noise consistency requirement. All Level 1 stale references (stage
number, validation command, PDF verification wording) are corrected.

**J2 dynamics mismatch additions (Stage 82):**

*j2Rk4 truth default* — `ScenarioPresets.singleAssetCarrierAttitude` now sets
`cfg.orbit.truth.mode = 'j2Rk4'` (was `twoBodyRk4`). EKF predictor remains `twoBody`.
Mismatch is intentional and documented: at GEO (~42164 km equatorial) the J2 perturbation
is ~8.3 × 10⁻⁶ m/s², well within the EKF's process-noise budget (`sigma_accel=0.01 m/s²`).

*Process-noise consistency audit* — `ConfigFactory` Stage 82 block verifies
`sigma_accel >= 0.1 × a_J2`. Auto-scales `modelMismatch.sigma_mps2 = max(1e-8, 0.25 × a_J2)`.

*Diagnostics fields* — `cfg.diagnostics.dynamicsMismatch.*` records policy, J2 accel norm,
process-noise consistency, and mismatch label. `sourceTruthStatus`, `eopStatus`, and
`reportStatusFreshnessStage` are declared.

All Stage 81 scientific closure, Stage 80 propagation/timing, and Stage 72-79 clock/slip/
covariance/ambiguity/central-config behaviour are preserved. Frozen report structure intact.
...
setenv('OO_V1_VALIDATE_REPORT', 'true')
setenv('OO_V1_ALL_TOGGLES',     'true')
setenv('OO_V1_VALIDATION_STAGE','82')
run_oo_reverse_gnss_report
```
...
Full suite is **NOT RUN**. Validation selects 2-5 tests only (default seed 82, default count 4).

### Stage 81 validation procedure (historical)
...
3. Verify the PDF: existing scientific summary fields and model-coverage table entries present; Numerical Summary is final section
4. Write `output/latest_validation_summary.json` and `.txt`
...
one-way timing consistency for the active single-space-asset, one-way reverse-GNSS simulation.
Central configuration remains the owner of all new variables.

**Propagation and timing changes (Stage 80):**

*Validation gate is active-scope aware (Stage 80)* — `MainScriptValidationGate` now excludes
inactive-feature tests (TWSTFT, ISL, two-way, multi-asset) from random test selection. These tests
remain in the repository as historical reference; only tests compatible with the active single-asset
one-way scope are selected. `validationScope='singleAssetOneWayActive'` is recorded in the JSON
summary and the report MAT file. Full suite NOT RUN.

*DiffAtt schema blocker fixed* — legacy differential-attitude stores are schema-complete after
initialization/finalization, including Stage 75/76 ambiguity-classification fields such as
`falseFixClassification`, `phaseBiasStatus`, `ambiguityStatus`, `dualFreqStatus`, and
`wideLaneStatus`.

*Truth/estimator propagation is configurable* — `cfg.orbit.truth.*` owns truth propagation,
`cfg.estimator.dynamics.*` owns the EKF predictor, and `j2Rk4` truth propagation is available
while the default validation run remains stable.

*One-way light-time metadata is implemented* — `cfg.physics.lightTime.*` controls receive/transmit
timing metadata. The active report can use iterative one-way timing while preserving code/carrier
geometry conventions.

*Sagnac double counting is guarded* — first-order Sagnac and geometric light-time Earth rotation
are mutually guarded. The report records the selected `sagnacHandling`.

*Doppler remains simplified v1* — unless explicitly upgraded, Doppler does not claim a full
transmit-time derivative.

IERS/EOP, SP3, CLK, RINEX, ANTEX, and IONEX remain not implemented. Existing Stage 72-79 physics,
central configuration ownership, one-space-asset topology, and the compact report structure are
preserved. The report still ends with Numerical Summary.

NOT ISL. NOT TWSTFT. NOT two-way. NOT multi-asset.
No LAMBDA/MLAMBDA. No carrier-IF integer fixing. No full WL/NL fixing.
No formal false-fix-risk probability. No calibrated phase-bias products.
...
setenv('OO_V1_VALIDATE_REPORT', 'true')
setenv('OO_V1_ALL_TOGGLES',     'true')
setenv('OO_V1_VALIDATION_STAGE','80')  % Stage 80 historical
run_oo_reverse_gnss_report
```

This will:
1. Select 4 tests (seed 80) from the active-scope pool (inactive TWSTFT/ISL/two-way/multi-asset tests excluded), stop if any fail
2. Run the main 3600 s report with all-toggles, dual-freq L1+L2 AR (via canonical enabledMask)
3. Verify the PDF: existing scientific summary fields present; Numerical Summary is final section
4. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 80, default count 4).
...
**Current stage:** Stage 84 — Doppler/Product-Covariance Correctness Hardening

Stage 84 hardens Doppler R covariance correctness and product-epoch drift semantics.
truth propagator. Representative J2 accel at GEO is recorded and process-noise consistency is
audited. All Stage 81 scientific closure additions are preserved.
See the Stage 82 validation procedure above for the full description.
...
The GEO satellite is propagated with `twoBodyRk4` truth propagator (Stage 67+). EKF uses matched `twoBody` predictor. GEO equatorial orbit: 35786 km, inclination 0°, ν₀ = 23°.
...
The compact PDF contains the frozen scientific report body: scenario summary, state-estimation
plots, measurement/geometry validation, receiver/per-receiver diagnostics, oscillator and clock
observability summaries, simulation physics/configuration tables, and Numerical Summary as the
final major section. Suppressed appendix-style diagnostic dumps are not part of the active report.
...
**Singularity:** The ZYX Euler kinematic equation is singular at pitch = ±90°. Stage 61 adds a quaternion error-state EKF parameterization that removes this singularity. Stage 62 fixes covariance reset ordering. Production-grade injection clamping and ring-down prevention are not implemented in v1.
...
| Limitation | Notes |
|-----------|-------|
| Carrier phase: L1/L2 float | `ekfFloat` mode supports carrier rows selected by canonical signal/carrier masks; no ionosphere-free combination |
...
| L1/L2 float carrier EKF | **Implemented** (ekfFloat mode; L2 selected by canonical masks) |
