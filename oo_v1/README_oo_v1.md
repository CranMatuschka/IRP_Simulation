...
| Item | Value |
|------|-------|
| Stage | **42** — L2 Carrier EKF Row Architecture v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 42; **full suite not run**) |
...
### Stage 42 validation procedure

Stage 42 adds `SignalCatalog` and `L2CarrierArchitectureDiagnostics`. Generalises the EKF
carrier ambiguity state indexing to support L1 and L2 float ambiguity states when the guarded
toggle `cfg.measurements.carrier.l2EkfRows.enable = true` is set. Updates
`CarrierMeasurementBuilder` to loop over signals from `SignalCatalog` (L1 always; L2 when
enabled), applying the correct wavelength and `(fL1/fL2)^2` ionosphere scaling per signal.
Updates `AmbiguityStateMetadata` to label states with signal IDs (`L1`, `L2`). Stage 42
architecture is float only: no ionosphere-free combination, no integer fixing, no
LAMBDA/MLAMBDA, no false-fix-risk control. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','42')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 42), force `test_stage42*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including L2 EKF rows)
4. Verify the PDF: scientific sections present, no "Stage 42 Validation Status" heading
...
**Current stage:** Stage 42 — L2 Carrier EKF Row Architecture v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 42, default count 4). Full suite is **NOT RUN**.
