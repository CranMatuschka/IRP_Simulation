...
| Item | Value |
|------|-------|
| Stage | **65** — Lean Scientific Report and Code Cleanup |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 65; **full suite not run**) |
...
### Stage 65 validation procedure

Stage 65 removes 2530 lines of stage-history and architecture-diary chapters from
`ClockExactReportBuilder.m`. The final PDF is now compact: scenario summary with measurement
model formulas, state estimation plots with 10%-tail zoom, measurement validation, oscillator
and clock diagnostics, numerical summary, and a final scientific closure table. Simulation
physics is unchanged.

Stage 65:
- removes 13 section-writing methods from ClockExactReportBuilder (~2530 lines deleted)
- suppresses from PDF: Model Reality Check, Multi-Asset, ISL, TWSTFT, Signal/Iono Architecture, Carrier Track Robustness, Troposphere Architecture, Bias Architecture, Disabled Components, Attitude Observability (Stage 14–64 diary)
- adds final-10%-zoom state estimate plots for position, clock bias, clock drift, and attitude
- extracts Stage 64 Final Scientific Closure into standalone section at end of report
- adds `cfg.report.{zoomLastFraction, compactFinalReport, suppressStageSections, deduplicateFigures}` flags
- simulation physics and all EKF logic unchanged
...
setenv('OO_V1_VALIDATION_STAGE','65')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 65), force `test_stage65*` if present (none exists; random selection used)
...
4. Verify the PDF: scientific sections present, Stage 65 in title
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 65, default count 4).
...
**Current stage:** Stage 65 — Lean Scientific Report and Code Cleanup

Stage 65 cleaned the report output (2530 lines removed from ClockExactReportBuilder),
while keeping all simulation physics intact. The PDF now shows only scientific essentials:
measurement model formulas, EKF state/measurement tables, state estimate plots with 10%
convergence zoom, residual/NIS diagnostics, numerical metrics, and a final scientific
closure table.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 65, default count 4). Full suite is **NOT RUN**.
