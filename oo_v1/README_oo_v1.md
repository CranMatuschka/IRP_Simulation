...
| Item | Value |
|------|-------|
| Stage | **50** — Ambiguity Fixing Readiness Gate v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 50; **full suite not run**) |
...
### Stage 50 validation procedure

Stage 50 adds a strict ambiguity fixing readiness gate (`AmbiguityFixingReadinessGate`)
that combines: carrier IF ambiguity pair metadata (Stage 48), Pamb covariance (Stage 41),
wide-lane/narrow-lane diagnostics (Stage 49), arc-quality availability (cycle-slip
detection), and residual/NIS consistency. This is a **readiness gate only** — no
ambiguity is fixed, rounded, or resolved. Integer fixing, LAMBDA/MLAMBDA, calibrated
...
setenv('OO_V1_VALIDATION_STAGE','50')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 50), force `test_stage50*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including readiness gate, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 50 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 50, default count 4).
...
**Current stage:** Stage 50 — Ambiguity Fixing Readiness Gate v1

Stage 50 adds a strict ambiguity fixing readiness gate combining carrier IF pair metadata
(Stage 48), Pamb covariance (Stage 41), wide-lane/narrow-lane float diagnostics (Stage 49),
arc-quality availability, and residual/NIS consistency. This is a readiness gate only —
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 50, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float IF rows; Stage 48 adds float traceability; Stage 49 adds float WL/NL diagnostics; Stage 50 adds readiness gate — no integer fixing, no calibrated phase-bias products, no PPP-grade)
