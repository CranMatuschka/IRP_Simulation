...
| Item | Value |
|------|-------|
| Stage | **39** — Carrier Row Metadata Inventory v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 39; **full suite not run**) |
...
### Stage 39 validation procedure

Stage 39 adds `CarrierRowMetadataInventory`: a carrier row metadata inventory helper that
reports carrier row counts, differential-attitude row counts, EKF vs diagnostic split, and
ambiguity state counts from existing summary metadata. Stage 39 is metadata inventory only:
no L2 carrier EKF, no ionosphere-free carrier, no integer ambiguity fixing, no attitude
accuracy claim. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','39')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 39), force `test_stage39*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled
4. Verify the PDF: scientific sections present, no "Stage 39 Validation Status" heading
...
**Current stage:** Stage 39 — Carrier Row Metadata Inventory v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 39, default count 4). Full suite is **NOT RUN**.
