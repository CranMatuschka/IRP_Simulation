...
| Item | Value |
|------|-------|
| Stage | **36** — Single-Asset Attitude Scenario Readiness Gate v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 36; **full suite not run**) |
...
### Stage 36 validation procedure

Stage 36 adds a readiness gate that combines receiver geometry rank, baseline structure,
measurement modes, attitude observability, Jacobian audit status, and attitude evidence
classification into a single `diagnostic-ready` / `carrier-float-ready` / etc. output.
This is a readiness gate only: no estimator redesign, no attitude accuracy claim, no
integer ambiguity fixing, no quaternion EKF, no operational certification.
The simulation duration remains 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','36')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 36), force `test_stage36*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled
4. Verify the generated PDF using `pdftotext` (TEX fallback if unavailable)
...
Outputs:
- `output/latest_validation_summary.json` — machine-readable summary (SHA, test results, PDF status)
