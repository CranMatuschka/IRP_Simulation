...
| Item | Value |
|------|-------|
| Stage | **56** — Measurement Geometry Core Consolidation v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 56; **full suite not run**) |
...
### Stage 56 validation procedure

Stage 56 consolidates duplicated measurement geometry and Jacobian logic from
`CodeJacobianBuilder` and `CarrierMeasurementBuilder` into a shared `LinkGeometry`
helper. Provides analytic LOS Jacobian, finite-difference position/attitude Jacobians,
and preferred attitude partial controls with backward-compatible legacy fallback.
The stale "Carrier IF not implemented" comment is replaced with a scientifically correct
guard. No measurement physics, EKF math, carrier arc consistency enforcement, or ambiguity
handling is changed. Integer fixing, LAMBDA/MLAMBDA, and false-fix-risk control are NOT
implemented in v1. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','56')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 56), force `test_stage56*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including LinkGeometry migration, plugin registry, arc consistency enforcement, arc-separated ambiguities, carrier arc evidence, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 56 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 56, default count 4).
...
**Current stage:** Stage 56 — Measurement Geometry Core Consolidation v1

Stage 56 consolidates duplicated measurement geometry and Jacobian logic from
`CodeJacobianBuilder` and `CarrierMeasurementBuilder` into a shared `LinkGeometry`
helper. Preferred attitude partial controls (`cfg.estimator.attitude.useCodePartials`,
`useCarrierPartials`, `useDopplerPartials`) are introduced with backward-compatible
legacy fallback. No measurement physics, EKF math, carrier arc consistency enforcement,
or ambiguity handling is changed. Integer fixing, LAMBDA/MLAMBDA, calibrated phase-bias
products, and false-fix-risk control are NOT implemented in v1.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 56, default count 4). Full suite is **NOT RUN**.
