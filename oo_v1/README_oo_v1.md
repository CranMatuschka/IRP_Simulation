...
| Item | Value |
|------|-------|
| Stage | **33** — Attitude Parameterization Convention Hardening |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 33; **full suite not run**) |
...
### Stage 33 validation procedure

Stage 33 hardens the attitude parameterization convention only: no quaternion EKF, no
attitude-sensitive Jacobian redesign, no attitude accuracy claim, no integer ambiguity fixing.
...
setenv('OO_V1_VALIDATION_STAGE','33')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 33), force `test_stage33*` if present
