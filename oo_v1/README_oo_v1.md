...
| Item | Value |
|------|-------|
| Stage | **37** — Move Validation Status Out of PDF Into README |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 37; **full suite not run**) |
...
### Stage 37 validation procedure

Stage 37 moves the validation-status chapter out of the PDF and into README and runtime
summary files. The PDF is now a scientific simulation report only. The simulation
duration remains 3600 s. No EKF changes, no carrier/L2 work, no quaternion EKF.
...
setenv('OO_V1_VALIDATION_STAGE','37')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 37), force `test_stage37*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled
4. Verify the PDF: scientific sections present, no "Stage 37 Validation Status" heading
...
**Current stage:** Stage 37 — Move Validation Status Out of PDF Into README
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 37, default count 4). Full suite is **NOT RUN**.
