...
| Item | Value |
|------|-------|
| Stage | **53** — Cycle-Slip-Aware Arc-Separated Float Ambiguities v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 53; **full suite not run**) |
...
### Stage 53 validation procedure

Stage 53 makes float carrier ambiguity handling cycle-slip-aware and arc-separated.
`CarrierTrackManager` is extended with a per-track arc ID (`currentArcId_`) so that
each cycle slip increments the arc ID, making arcs within a track distinguishable.
`AmbiguityArcState` collects arc metadata from `cpInfo` after slip detection and
exposes per-row arc IDs, arc epoch counts, and slip counts to the summary. Arc
consistency is checked for carrier IF and WL/NL pairs (L1 and L2 must share the same
arc ID). This is still a **float-only diagnostic** — no ambiguity is fixed, rounded,
or resolved. Integer fixing, LAMBDA/MLAMBDA, calibrated phase-bias products, and
false-fix-risk control are NOT implemented in v1. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','53')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 53), force `test_stage53*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including arc-separated ambiguities, carrier arc evidence, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 53 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 53, default count 4).
...
**Current stage:** Stage 53 — Cycle-Slip-Aware Arc-Separated Float Ambiguities v1

Stage 53 makes float carrier ambiguity handling cycle-slip-aware by separating ambiguity
arcs after slips and preventing carrier ionosphere-free or wide-lane/narrow-lane diagnostics
from silently bridging incompatible arcs. Each cycle slip on a track increments its arc ID;
`AmbiguityArcState` exposes per-row arc metadata (arc ID, arc epoch count, slip count) in the
summary. Arc consistency is checked for carrier IF and WL/NL pairs. This is still a float-only
diagnostic — no ambiguity is fixed or rounded. Integer fixing, LAMBDA/MLAMBDA, calibrated
phase-bias products, and false-fix-risk control are NOT implemented in v1.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 51, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float IF rows; Stage 48 adds float traceability; Stage 49 adds float WL/NL diagnostics; Stage 50 adds readiness gate; Stage 51 hardens readiness evidence; Stage 52 exports arc evidence; Stage 53 adds arc-separated float ambiguities — no integer fixing, no calibrated phase-bias products, no PPP-grade)
