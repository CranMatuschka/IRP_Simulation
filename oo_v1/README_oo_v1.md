...
| Item | Value |
|------|-------|
| Stage | **45** — Guarded Ionosphere-Free Code EKF Rows v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 45; **full suite not run**) |
...
### Stage 45 validation procedure

Stage 45 adds guarded L1/L2 ionosphere-free code EKF rows: `CodeIonoFreeRowBuilder` for row
combination, `CodeIonoFreeEkfDiagnostics` for status reporting, and wires the new
`cfg.measurements.code.ionosphereFreeRows.enable/useInEkf` toggles into the existing IF
combination path in `CodeMeasurementBuilder`. This is **code IF only**: no carrier IF rows,
no integer ambiguity fixing, no LAMBDA/MLAMBDA, no PPP-grade claim. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','45')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 45), force `test_stage45*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including code IF EKF rows)
4. Verify the PDF: scientific sections present, no "Stage 45 Validation Status" heading
...
**Current stage:** Stage 45 — Guarded Ionosphere-Free Code EKF Rows v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 45, default count 4). Full suite is **NOT RUN**.
...
- EKF use of ionosphere-free carrier rows (carrier IF, P3-like carrier; Stage 45 adds code IF EKF rows only: no carrier IF, no integer fixing, no LAMBDA/MLAMBDA, no higher-order iono, no PPP-grade)
