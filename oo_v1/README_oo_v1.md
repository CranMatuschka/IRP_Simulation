...
| Item | Value |
|------|-------|
| Stage | **78** — Central Configuration Completion and Source-Truth Cleanup |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 78; **full suite not run**) |
...
### Stage 78 validation procedure

Stage 78 completes central configuration source-truth cleanup. All hardcoded frequency constants
outside `SignalDefinition.m` and `ConfigFactory.defaultConfig()` are replaced with canonical
`SignalDefinition` lookups or `cfg.signals.frequencyHz/wavelength_m` arrays. Silent multi-asset
truncation in `ScenarioPresets` is replaced with a policy-guarded error. Stale scientific comments
corrected. `cfg.signals.wavelength_m` added as a new canonical output from `finalizeConfig`.
Legacy per-signal aliases (`cfg.signals.L1.frequency_Hz` etc.) derived in `finalizeConfig`.
Stage 77 canonical chain and Stages 72–77 baselines fully preserved.

**Source-truth changes (Stage 78):**

*Frequency constants removed from computation code* — `BaselineCarrierAmbiguityResolver`,
`CodeMeasurementBuilder`, `ClockExactReportBuilder`, and `ReportRunner` no longer contain
hardcoded `1575.42e6` / `1227.60e6` frequency constants. All replaced with
`revgnss.SignalDefinition.get('L1/L2')` calls or canonical `cfg.signals.frequencyHz/wavelength_m`.

*`cfg.signals.wavelength_m` added to `finalizeConfig`* — new canonical field parallel to
`cfg.signals.frequencyHz`, populated for all enabled signals from `SignalDefinition`.

*Legacy aliases derived in `finalizeConfig`* — `cfg.signals.L1.frequency_Hz/lambda_m` etc.
are now derived from the canonical arrays, so legacy code continues to work without hardcoding.

*Unknown signal now errors* — `ConfigFactory.finalizeConfig` signal centralization block drops
the `try/catch` fallback that silently defaulted to `1575.42e6`. `SignalDefinition.get()` already
errors on unknown signals; that error now propagates cleanly.

*Multi-asset truncation is guarded* — `ScenarioPresets.singleAssetCarrierAttitude` replaces
silent `cfg.assets = cfg.assets(1)` with a policy-guarded error. Errors by default;
warns-and-truncates only when `cfg.validation.unsupportedFeaturePolicy='disableWithWarning'`.

**Summary fields added (Stage 78):**
`staleSourceTruthBlocksRemoved`, `signalConfigOwner`, `frequencyHardcodeAuditStatus`,
...
`nCanonicalWarnings`, `nCanonicalErrors` — added to existing Simulation Physics table
("Source truth (Stage 78)" row), no new report chapter.
...
setenv('OO_V1_VALIDATION_STAGE','78')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 78), stop before report generation if any selected test fails
...
Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 78, default count 4).
...
**Current stage:** Stage 78 — Central Configuration Completion and Source-Truth Cleanup

Stage 78 completes source-truth cleanup: all hardcoded frequency constants outside
`SignalDefinition.m` are removed from computation code; `cfg.signals.wavelength_m` added;
legacy aliases derived in `finalizeConfig`; multi-asset truncation guarded.
See the Stage 78 validation procedure above for the full description.
