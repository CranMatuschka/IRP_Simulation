...
| Item | Value |
|------|-------|
| Stage | **38** — Carrier-Phase Attitude Preparation v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 38; **full suite not run**) |
...
### Stage 38 validation procedure

Stage 38 adds `CarrierAttitudePreparation`: a carrier-phase audit helper that classifies the
current measurement stack relative to attitude-mode selection. Reads `totalCarrierRows`,
`totalDiffAttRows`, `nTowers`, and `nReceivers` from the run summary. L2 carrier EKF,
integer fixing, and quaternion states are not implemented. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','38')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 38), force `test_stage38*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled
4. Verify the PDF: scientific sections present, no "Stage 38 Validation Status" heading
...
**Current stage:** Stage 38 — Carrier-Phase Attitude Preparation v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 38, default count 4). Full suite is **NOT RUN**.
