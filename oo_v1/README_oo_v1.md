...
| Item | Value |
|------|-------|
| Stage | **29** — Main-Script Validation Freshness Gate |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 29; **full suite not run**) |
...
setenv('OO_V1_VALIDATION_STAGE','29')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 29), force `test_stage29*` if present
...
> the repository as historical reference only. They are no longer the active validation entry
> point for Stage 29+.
