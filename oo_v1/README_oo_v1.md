...
| Item | Value |
|------|-------|
| Stage | **41** — Ambiguity State Metadata and Covariance Export v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 41; **full suite not run**) |
...
### Stage 41 validation procedure

Stage 41 adds `AmbiguityStateMetadata`: exports EKF float ambiguity state-map indices and
the final ambiguity covariance sub-block diagnostics into the runtime report output.
Updates `CarrierRowMetadataInventory` to prefer state-map-derived ambiguity counts and
`AmbiguityReadinessDiagnostics` to prefer the exported covariance summary. Fixes the
known-ambiguity validation classification priority. Stage 41 is metadata/covariance export
only: no integer fixing, no LAMBDA/MLAMBDA, no L2 carrier EKF, no false-fix-risk control,
no attitude accuracy claim. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','41')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 41), force `test_stage41*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled
4. Verify the PDF: scientific sections present, no "Stage 41 Validation Status" heading
...
**Current stage:** Stage 41 — Ambiguity State Metadata and Covariance Export v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 41, default count 4). Full suite is **NOT RUN**.
