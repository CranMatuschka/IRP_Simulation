...
| Item | Value |
|------|-------|
| Stage | **77** — Central Configuration Source of Truth |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 77; **full suite not run**) |
...
### Stage 77 validation procedure

Stage 77 makes `ConfigFactory.finalizeConfig()` the single canonical source for all scientific
configuration parameters. Four fragmented duplicate assignments are removed from the run script;
`finalizeConfig` derives all downstream values from canonical sources. Stage 76 signal/AR/dimension
logic and Stages 72–76 baselines are fully preserved.

**Configuration centralization (Stage 77):**

*Single signal control* — `cfg.signals.enabledMask` replaces `cfg.signals.twoFrequency.enable` as
the canonical frequency selector. `finalizeConfig` derives `cfg.signals.enabled`, `twoFrequency.enable`,
`code.enabledByFrequency`, `carrier.enabledByFrequency`, and `diffAtt.ambiguityResolution.enabledByFrequency`
from it. Changing `[true, true]` → `[true, false]` in one place makes the run L1-only.

*Receiver count — single owner* — `cfg.scenario.nReceivers = 3` removed from run script.
`ScenarioPresets.singleAssetCarrierAttitude` is the single canonical owner (sets 4).
No hidden 3→preset→4 confusion.

*Slip threshold — single owner* — `cfg.carrierSlip.threshold_m` is canonical.
`cfg.measurements.carrier.slipDetection.threshold_m` (read by `CarrierTrackManager`) is derived
from it in `finalizeConfig`. Explicit redundant assignment removed from run script.

*Clock product mode — single owner* — `cfg.clocks.tower.product.mode` is canonical.
`cfg.errors.towerClockCorrection.mode` (legacy alias → `cfg.estimator.towerClockMode`) is derived
from it in `finalizeConfig` before the existing legacy mapping. Redundant assignment removed.

*AR enabledByFrequency — derived* — `cfg.estimator.diffAtt.ambiguityResolution.enabledByFrequency`
is derived from `cfg.signals.enabledMask` in `finalizeConfig` when not explicitly sized to match
the signal count. Explicit assignment removed from run script.

**Summary fields added (Stage 77):**
`centralConfigStatus`, `signalMaskCanonicalOwner`, `nReceiversCanonicalOwner`,
`slipThresholdCanonicalOwner`, `clockProductModeCanonicalOwner`, `arByFreqDerivedFrom`,
`canonicalSlipThreshold_m`, `canonicalSignalEnabledMask` — all in existing Simulation Physics
table (one new row: "Config source (Stage 77)"), no new report chapter.

**(A) Stage 72/73/74/75/76 baselines preserved** — `truthHistoryProductNoisy`, compensated slip
detection, block covariance, per-baseline classification, dual-freq AR. Performance unchanged.
...
setenv('OO_V1_VALIDATION_STAGE','77')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 77), stop before report generation if any selected test fails
...
Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 77, default count 4).
