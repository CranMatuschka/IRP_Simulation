...
| Item | Value |
|------|-------|
| Stage | **54** — Enforced Arc-Consistent Carrier Combinations v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 54; **full suite not run**) |
...
### Stage 54 validation procedure

Stage 54 enforces arc consistency for carrier ionosphere-free and wide-lane/narrow-lane
carrier combinations. L1/L2 carrier rows from incompatible ambiguity arcs are skipped
or rejected instead of being silently combined. When `cfg.estimator.enforceCarrierArcConsistency.enable=true`,
`CarrierIonoFreeRowBuilder` skips arc-inconsistent L1/L2 pairs under the
`disableWithWarning` policy, or throws a typed error under the `error` policy.
Wide-lane/narrow-lane diagnostics are blocked or downgraded when arc-inconsistent pairs
exist. This is a float measurement-model correctness step — no ambiguity is fixed,
rounded, or resolved. Integer fixing, LAMBDA/MLAMBDA, calibrated phase-bias products,
...
setenv('OO_V1_VALIDATION_STAGE','54')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 54), force `test_stage54*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including arc consistency enforcement, arc-separated ambiguities, carrier arc evidence, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 54 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 54, default count 4).
...
**Current stage:** Stage 54 — Enforced Arc-Consistent Carrier Combinations v1

Stage 54 enforces arc consistency for carrier ionosphere-free and wide-lane/narrow-lane
carrier combinations. L1/L2 rows from incompatible ambiguity arcs are skipped or rejected
instead of being silently combined, and wide-lane/narrow-lane diagnostics are blocked or
downgraded when arc-inconsistent pairs exist. This is a float measurement-model correctness
step — no ambiguity is fixed or rounded. Integer fixing, LAMBDA/MLAMBDA, calibrated
