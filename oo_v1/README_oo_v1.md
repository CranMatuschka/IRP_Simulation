...
| Item | Value |
|------|-------|
| Stage | **32** — Single-Asset Receiver Geometry Model v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 32; **full suite not run**) |
...
### Stage 32 validation procedure

Stage 32 adds receiver geometry bookkeeping only: no attitude accuracy claim, no antenna
calibration (PCO/PCV/ANTEX), and no integer ambiguity fixing.
...
setenv('OO_V1_VALIDATION_STAGE','32')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 32), force `test_stage32*` if present
