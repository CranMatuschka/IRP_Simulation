...
| Item | Value |
|------|-------|
| Stage | **57** — EKF Innovation Accounting and Gauge/NIS Cleanup v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 57; **full suite not run**) |
...
### Stage 57 validation procedure

Stage 57 separates the EKF innovation NIS into physical (real measurement rows) and gauge
(datum/clock constraint rows appended before update) contributions. The
`EkfInnovationAccounting` helper provides `classifyRows`, `compute`, `residualRms`, and
`compact` static methods. `ReverseGNSSSimulation` captures `nu` and `S` from `update()` to
compute separated accounting per epoch. `Diagnostics` stores per-epoch Stage 57 fields;
`ReportRunner` exposes `physicalNIS`, `gaugeNIS`, and per-type RMS in the summary struct.
The existing `summary.meanNIS` is retained as a legacy alias
(`legacyMeanNisIncludesGauge = true`). No measurement physics, EKF update math, carrier arc
consistency enforcement, or ambiguity handling is changed. Integer fixing, LAMBDA/MLAMBDA,
and false-fix-risk control are NOT implemented in v1. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','57')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 57), force `test_stage57*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including EKF innovation accounting, LinkGeometry migration, plugin registry, arc consistency enforcement, arc-separated ambiguities, carrier arc evidence, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 57 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 57, default count 4).
...
**Current stage:** Stage 57 — EKF Innovation Accounting and Gauge/NIS Cleanup v1

Stage 57 separates the EKF innovation NIS into physical and gauge contributions.
`EkfInnovationAccounting` helper provides classifyRows/compute/residualRms/compact.
`physicalNIS` (real measurement rows only) is exposed in summary; `legacyMeanNisIncludesGauge=true`
marks the existing `meanNIS` as augmented. No measurement physics, EKF update math, carrier arc
consistency enforcement, or ambiguity handling is changed. Integer fixing, LAMBDA/MLAMBDA,
calibrated phase-bias products, and false-fix-risk control are NOT implemented in v1.
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 57, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float IF rows; Stage 48 adds float traceability; Stage 49 adds float WL/NL diagnostics; Stage 50 adds readiness gate; Stage 51 hardens readiness evidence; Stage 52 exports arc evidence; Stage 53 adds arc-separated float ambiguities; Stage 54 enforces arc-consistent carrier combinations; Stage 55 is architecture cleanup; Stage 56 is geometry core consolidation; Stage 57 is EKF innovation accounting — no integer fixing, no calibrated phase-bias products, no PPP-grade)
