...
| Item | Value |
|------|-------|
| Stage | **49** — Wide-Lane / Narrow-Lane Float Diagnostics v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 49; **full suite not run**) |
...
### Stage 49 validation procedure

Stage 49 adds float wide-lane and narrow-lane ambiguity-combination diagnostics from the
traced L1/L2 ambiguity covariance exported by Stage 41.
`WideLaneNarrowLaneDiagnostics` computes cycle-domain sigma diagnostics and the
wide-lane/narrow-lane correlation using
`D = [1/lambda1, -1/lambda2; 1/lambda1, 1/lambda2]` and `P_N = D * P_pair * D'`.
This is **float diagnostics only** — no ambiguity is fixed or rounded,
no LAMBDA/MLAMBDA, no calibrated phase-bias products, no false-fix-risk control.
...
setenv('OO_V1_VALIDATION_STAGE','49')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 49), force `test_stage49*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 49 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 49, default count 4).
...
**Current stage:** Stage 49 — Wide-Lane / Narrow-Lane Float Diagnostics v1

Stage 49 adds float wide-lane and narrow-lane ambiguity-combination diagnostics from
the traced L1/L2 ambiguity covariance (Stage 41). This is float diagnostics only —
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 49, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float IF rows; Stage 48 adds float traceability; Stage 49 adds float WL/NL diagnostics — no integer fixing, no calibrated phase-bias products, no PPP-grade)
