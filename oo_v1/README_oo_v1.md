...
| Item | Value |
|------|-------|
| Stage | **64** — Scientific Closure and v1 Freeze |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 64; **full suite not run**) |
...
### Stage 64 validation procedure

Stage 64 is the **v1 scientific closure and freeze**. It documents known scientific limitations
explicitly, corrects the PCV default to `none`, labels the IF covariance assumption
(Cov(L1,L2)=0) and the simplified v1 Doppler model, and adds a Final Scientific Closure
section to the PDF. v1 is now frozen as a controlled, internally consistent MATLAB EKF
simulation demonstrating measurement, covariance, ambiguity-state, attitude, dynamics, and
reporting architecture under selected synthetic scenarios.

Stage 64:
- corrects PCV default from `'toy'` to `'none'`; toy PCV is synthetic-only, never active by default
- documents IF variance formula: `Var(IF) = alpha^2*Var(L1) + beta^2*Var(L2)`, `Cov(L1,L2)=0`
- labels Doppler as "simplified v1": LOS range-rate + rx/tower clock drift; no Sagnac-rate, no relativistic range-rate, no lever-arm velocity from body rates
- adds Stage 64 Final Scientific Closure section in PDF with closure table
- adds stage64* summary fields in ReportRunner
- acknowledges Stage 63 active-no-fixes as correct (not a failure) in all-toggle/IF mode
- Stage 61/62 quaternion covariance update-order preserved
- full suite NOT RUN; v1 frozen
...
setenv('OO_V1_VALIDATION_STAGE','64')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 64), force `test_stage64*` if present
...
4. Verify the PDF: scientific sections present, Stage 64 section present
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 64, default count 4).
...
**Current stage:** Stage 63 — Controlled Single-Asset Integer Ambiguity Fixing v1

Stage 63 adds guarded raw-carrier integer ambiguity fixing for the controlled
`singleAssetCarrierAttitude` scenario (`floatPerTowerReceiverSignal` mode).
Fixes pass 18 scientific guards and are applied as EKF pseudo-measurements.
Stage 62 `quaternionErrorState` covariance consistency is preserved.
Stage 59 single-space-asset scenario and Stage 60 carrier-attitude closure are preserved.
Full suite is **NOT RUN**.

Stage 63 does not implement:
- LAMBDA/MLAMBDA integer ambiguity resolution
- Carrier ionosphere-free (IF) integer fixing
- Wide-lane / narrow-lane integer fixing
- Formal false-fix-risk control or ratio/residual validation
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 63, default count 4). Full suite is **NOT RUN**.
