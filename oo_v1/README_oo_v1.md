...
| Item | Value |
|------|-------|
| Stage | **25-26** — Script-Exact Validation Gate + Orbit Dynamics Foundation |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 25; **full suite not run**) |
...
> **Note:** Stages 25-26 deliberately do not run the full test suite.
> The authoritative commit SHA and test pass/fail count are written to
> `output/latest_validation_summary.json` at runtime by `run_stage25_26_validation.m`.
> Do not hard-code a commit SHA in this README — it becomes stale on the next commit.

### Stage 25-26 validation procedure

```matlab
cd oo_v1
run_stage25_26_validation   % selects 4 random tests (seed 25), runs main script with all toggles, verifies PDF
```
...
- `output/notion_stage25_26_update.md` — ready-to-paste Notion update block
...
+revgnss/
  ...
  OrbitPropagator.m    Circular/two-body/J2 orbit propagator (ECI -> ECEF)
...
- Higher-fidelity orbit: drag, SRP, third bodies beyond two-body+J2 (Stage 26)
