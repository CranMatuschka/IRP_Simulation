...
| Item | Value |
|------|-------|
| Stage | **55** — Source Truth and Report Architecture Cleanup v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 55; **full suite not run**) |
...
### Stage 55 validation procedure

Stage 55 is a source-truth and report-architecture cleanup. It introduces `StageHistory`
and `DiagnosticPluginRegistry` helpers, delegates history/missing-stage lists from
`ReportStatus` to `StageHistory`, and adds `DiagnosticPluginRegistry.collectAll` to
`ReportRunner` for plugin metadata. No measurement physics, EKF math, carrier arc
...
setenv('OO_V1_VALIDATION_STAGE','55')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 55), force `test_stage55*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including plugin registry, arc consistency enforcement, arc-separated ambiguities, carrier arc evidence, WL/NL diagnostics, carrier IF rows, Stage 41 ambiguity metadata, and traceability)
4. Verify the PDF: scientific sections present, no "Stage 55 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Validation selects 2–5 tests only (default seed 55, default count 4).
...
**Current stage:** Stage 55 — Source Truth and Report Architecture Cleanup v1

Stage 55 cleans source-truth, validation-status, and report/diagnostic architecture.
`StageHistory` and `DiagnosticPluginRegistry` helpers are introduced; `ReportStatus`
delegates long history/missing-stage lists to `StageHistory`; `ReportRunner` calls
`DiagnosticPluginRegistry.collectAll` for plugin metadata. No measurement physics,
EKF math, carrier arc consistency enforcement, or ambiguity handling is changed.
Integer fixing, LAMBDA/MLAMBDA, calibrated phase-bias products, and false-fix-risk
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 55, default count 4). Full suite is **NOT RUN**.
...
- Integer ambiguity fixing for carrier IF (LAMBDA/MLAMBDA; Stage 47 adds float IF rows; Stage 48 adds float traceability; Stage 49 adds float WL/NL diagnostics; Stage 50 adds readiness gate; Stage 51 hardens readiness evidence; Stage 52 exports arc evidence; Stage 53 adds arc-separated float ambiguities; Stage 54 enforces arc-consistent carrier combinations; Stage 55 is architecture cleanup — no integer fixing, no calibrated phase-bias products, no PPP-grade)
