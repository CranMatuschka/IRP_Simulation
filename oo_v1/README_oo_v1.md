...
| Validation mode | Targeted random smoke (2–5 selected tests; **full suite not run**) |
...
> **Note:** Stage 24 deliberately does not run the full test suite.
> The authoritative commit SHA and test pass/fail count are written to
> `output/latest_validation_summary.json` at runtime by `run_stage24_validation.m`.
> Do not hard-code a commit SHA in this README — it becomes stale on the next commit.

### Stage 24 validation procedure

```matlab
cd oo_v1
run_stage24_validation      % selects 2-5 random tests, runs all-toggle report, verifies PDF
```
...
- `output/notion_stage24_update.md` — ready-to-paste Notion update block
...
```matlab
% In run_oo_reverse_gnss_report.m, set:
