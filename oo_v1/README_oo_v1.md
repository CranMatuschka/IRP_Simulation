...
| Item | Value |
|------|-------|
| Stage | **43** — Ionosphere-Free Combination Diagnostics v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 43; **full suite not run**) |
...
### Stage 43 validation procedure

Stage 43 adds `IonosphereFreeCombinationDiagnostics` — a diagnostic-only helper that computes
L1/L2 ionosphere-free combination coefficients (alpha, beta), verifies first-order ionosphere
cancellation numerically, reports noise amplification (~2.98× for equal sigma), and documents
the IF carrier ambiguity caveat. This is **diagnostic only**: the IF combination is not fed
into the EKF, no integer ambiguity fixing is performed, and no LAMBDA/MLAMBDA is implemented.
No PPP-grade claim is made. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','43')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 43), force `test_stage43*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled (including IF combination diagnostic)
4. Verify the PDF: scientific sections present, no "Stage 43 Validation Status" heading
5. Write `output/latest_validation_summary.json` and `.txt`

Full suite is **NOT RUN**. Runtime summaries are local evidence only — never committed.
...
**Current stage:** Stage 43 — Ionosphere-Free Combination Diagnostics v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 43, default count 4). Full suite is **NOT RUN**.
...
- EKF use of ionosphere-free code/carrier rows (IF row construction, P3/L3 observables; Stage 43 is diagnostic-only: no EKF IF rows, no integer fixing, no LAMBDA/MLAMBDA, no higher-order iono, no PPP-grade)
