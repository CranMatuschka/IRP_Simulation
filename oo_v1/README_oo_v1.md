...
| Item | Value |
|------|-------|
| Stage | **40** — Ambiguity Readiness Diagnostics v1 |
| Branch | `feature/oo-reverse-gnss-v1` |
| Validation mode | Targeted random smoke (4 selected tests, seed 40; **full suite not run**) |
...
### Stage 40 validation procedure

Stage 40 adds `AmbiguityReadinessDiagnostics`: float ambiguity readiness assessment using
the Stage 39 carrier row inventory, covariance availability check, slip detection status,
and known-ambiguity validation flag. Stage 40 is readiness diagnostics only: no integer
fixing, no LAMBDA/MLAMBDA, no L2 carrier EKF. Simulation duration: 3600 s.
...
setenv('OO_V1_VALIDATION_STAGE','40')
run_oo_reverse_gnss_report
```

This will:
1. Select 4 random tests (seed 40), force `test_stage40*` if present
2. Stop before report generation if any selected test fails
3. Run the main 3600 s report with all-toggles enabled
4. Verify the PDF: scientific sections present, no "Stage 40 Validation Status" heading
...
**Current stage:** Stage 40 — Ambiguity Readiness Diagnostics v1
...
**Validation mode:** Targeted random smoke — 2–5 selected tests only (default seed 40, default count 4). Full suite is **NOT RUN**.
