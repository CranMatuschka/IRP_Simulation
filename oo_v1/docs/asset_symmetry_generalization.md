...
2. **3b-2 — route secondaries through it (FAITHFUL retirement).** Call the generalized builder per estimated secondary (`assetIdx=2..N`, `role='towerDownlink'`) via a `SecondaryMeasurementProfile` that reproduces the retired builder's exact realization; retire `SecondaryGroundMeasurementBuilder`. Preserve the pre-merge row order (chief rows, then per-secondary blocks) — swarm bit-identity. Fold `secondaryZwd`/Guard-A into the shared path. **Detailed staged plan: §15 (C0–C5).** Does NOT change swarm physics.
