...
|---|-----|---------|-------|--------|
| F1 | H | FFT colored-noise amplitude 2/N too small — flicker floor absent from every archived run incl. golden references | `+models/+clocks/ClockModel.m:214` | Confirmed; **fix present in working tree, uncommitted**; DC-bin `abs()` residual remains |
...
| F4 | H | `towerClockMode='perfectCorrection'` default = truth-assisted tower clock (error row zero by construction) | `masterConfig.m:1740`, `TowerClockCorrectionProvider.m:89-90` | Disclosed; baseline label required in any publication |
...
| F13 | M | Periodic relativistic clock term (e·sinE) absent: 0.3–3 ns (9 cm–1 m) orbit-periodic for real GEO e=1e-4–1e-3, aliasing into the radial-clock weak direction | absent | Should be added to ERROR_BUDGET absent list with magnitude |
...
| F23 | M | J2 auto-tuner in `finalizeConfig` force-enables `modelMismatch` and **silently overwrites** `sigma_mps2 ≤ 1e-6` (incl. the shipped default) with `max(1e-8, 0.25·|a_J2|)` — no warning; masterConfig text alone under-determines the run | `ConfigFactory.m:1875-1886` | Confirmed still present; bounded (inert on default `stationaryEcef` path; resolved cfg persisted; values >1e-6 respected) |
| F24 | H | Monte-Carlo NEES/NIS machinery is correct (two-sided χ², proper pooling, matched-crutch refusal) but **off by default** — the shipped consistency verdict is a single χ² sample; validation in the NASA-STD-7009 sense has not been performed and the repo's own manifest says `declaredNotStatisticallyExecuted` | `MonteCarloConsistency.m`, `masterConfig.m:435-460` | Machinery ✅, default execution gap; must be stated in any publication |
...
| F28 | M | Star-tracker noise isotropic (10″ per axis) — ignores the 5–10× boresight/twist anisotropy every unit in the NASA reference exhibits; error geometry spherical instead of cigar-shaped, which matters for lever-arm/attitude observables | `masterConfig.m:248`, `StarTrackerMeasurementModel.m` | Interface already supports full 3×3 covariance — parameterization gap only |
