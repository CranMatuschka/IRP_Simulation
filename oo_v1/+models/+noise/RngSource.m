classdef RngSource
    ...
    properties (Constant)
        ...
        TOWER_SECONDARY = 20  % ground-tower -> secondary-asset code thermal noise (white)
        ...
        ISL_TWOWAY_THERMAL  = 22  % all-pairs two-way baseline thermal noise (white, per-pair/per-epoch)
        ISL_TWOWAY_DELAYCAL = 23  % per-link turn-around+antenna-PCO delay-cal bias (interval-keyed: sig 0=const, 1=RW)
        SEC_TWSTFT_TWOWAY   = 24  % ground-tower <-> SECONDARY two-way time-transfer noise (white, per-tower/asset/epoch)
