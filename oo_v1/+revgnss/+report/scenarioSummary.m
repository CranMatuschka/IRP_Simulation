...
%   method on ClockExactReportBuilder) and is invoked below via CE.writeComponentRows_.
    ...
    if nargin >= 12 && ~isempty(figDir)
        ...
        if ~isempty(scFig)
            ...
            CE.writeRow_(fid, scFig, 'Space Asset and Reference Frames', ...
            ...
            fprintf(fid, ['\\noindent \\textit{Reference-frame schematic: run make\\_utils\\_figures.m to ' ...
                'generate output/utils/spacecraft\\_frames.pdf.}\n\n']);
        end
    end
