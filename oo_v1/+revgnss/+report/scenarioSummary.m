...
%   method on ClockExactReportBuilder) and is invoked below via CE.writeComponentRows_.
    ...
    % Spacecraft body + reference-frame schematic
    if nargin >= 12 && isstruct(plotPaths)
        fprintf(fid, CE.plotTableHeader_());
        CE.writeRow_(fid, CE.figRef_(plotPaths, 'scGeom', figDir, stem), ...
            'Space Asset and Reference Frames', ...
            ['Stylised schematic (not to scale) of the primary space asset --- an octagonal ' ...
             'bus with two octagonal solar panels. The ECI (inertial) and ECEF (Earth-fixed) ' ...
             'frames are drawn at Earth; the RAC (radial / along-track / cross-track orbital) ' ...
             'and body-attitude frames at the spacecraft. For an equatorial GEO the cross-track ' ...
             'axis coincides with the ECI spin axis. When a multi-asset swarm is active, the ' ...
             'helix formation of the secondary assets is overlaid.']);
