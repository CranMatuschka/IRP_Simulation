...
%   revgnss.ReportLabel so no raw CamelCase config string reaches the reader.
    ...
    % Relativistic clock-rate offset: modelled as a gated truth-side term, default off;
    % the constant offset is absorbed by the estimated drift for a circular orbit.
    fprintf(fid, ['Relativistic clock-rate offset (gravitational + special-relativistic, ' ...
        ...
        'modelled as a gated truth-side term on the receiver clock (default off). The constant offset is ' ...
        'observable and absorbed by the estimated clock-drift state for a circular orbit, so it leaves zero ' ...
        'estimation residual; only an eccentric orbit''s periodic term would survive.\\\\\n']);
