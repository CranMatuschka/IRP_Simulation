...
%     .nAssets .nTowers .nReceivers .duration_s
    ...
    if shapeOn
        layerText = ['the two-way ISL / sat--sat TWSTFT relative layer is a read-only ' ...
            'post-processor: it sharpens observable formation shape and relative clocks'];
    else
    ...
    if shapeOn
        fprintf(fid, ['{\\footnotesize $^{\\ast}$reference asset. \\textbf{relPos raw} = each satellite''s ' ...
            ...
            'Both are \\emph{truth-free} (ref-differenced in the estimate frame; truth is used only to score the ' ...
            'error), so the raw$\\rightarrow$solved change is the genuine ISL \\emph{shape} sharpening. Tail-averaged ' ...
            '(last 20\\,\\%%).}\n\n']);
    else
        fprintf(fid, ['{\\footnotesize $^{\\ast}$reference asset. \\textbf{relPos raw} = each satellite''s ' ...
            'relative-position error vs the reference from differencing the two \\emph{independent} EKF absolutes. ' ...
