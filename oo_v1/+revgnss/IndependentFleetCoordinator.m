classdef IndependentFleetCoordinator < handle
    ...
    methods
        ...
        function initialize(obj)
            ...
            if strcmp(settings.correlationNetwork.policy,'exactPairwiseCrossCovariance')
                ...
                % Section 3.3: declare the common-process-noise group on both consumers -- the
                % network's own cross-block propagation (declareCommonProcessNoiseGroup) AND
                % every leaf's own buildQ_ diagonal (localSim.ekf.declaredCommonProcessNoise
                % Group_) -- from ONE construction below, assigned to both. revgnss.
                % CommonProcessNoiseCovarianceGroup is a MATLAB value class with every property
                % SetAccess=immutable and no mutators, so there is no handle-style "same instance"
                % to alias; what actually prevents drift is that both consumers read the identical
                % (isequal) value from this one construction site, and neither can mutate it.
                % No-op (obj.correlationNetwork_.commonProcessNoiseGroups_ stays empty, every
                % leaf's declaredCommonProcessNoiseGroup_ stays empty) unless explicitly
                % configured; requireCorrelationNetworkConfiguration_ already refuses this
                % treatment together with policy='disabled'.
                if strcmp(settings.correlationNetwork.commonProcessNoiseTreatment, ...
                        'declaredCommonAccelerationGroup')
                    memberIdentifiers = arrayfun(@(a) sprintf('spacecraft:%d',a), ...
                        1:obj.nAssets,'UniformOutput',false);
                    commonProcessNoiseGroup = revgnss.CommonProcessNoiseCovarianceGroup.fromRecord(struct( ...
                        'processNoiseGroupIdentifier','fleetGroup:sharedForceAtmosphericProduct', ...
                        'commonSourceName','sharedForceAtmosphericProduct', ...
                        'treatment','declaredCommonAccelerationGroup', ...
                        'memberEndpointIdentifiers',{memberIdentifiers}, ...
                        'frameIdentifier',settings.correlationNetwork.commonProcessNoise.frame, ...
                        'commonAccelerationSigma_mps2',settings.correlationNetwork.commonProcessNoise.sigma_mps2, ...
                        'stateComponentPairing','positionVelocityPerAxis', ...
                        'sourceConfigurationPath', ...
                            'multiAsset.distributedEstimator.correlationNetwork.commonProcessNoise', ...
                        'validFromCoordinateEpoch_s',obj.tVec(1),'validUntilCoordinateEpoch_s',obj.tVec(end)));
                    obj.correlationNetwork_.declareCommonProcessNoiseGroup(commonProcessNoiseGroup);
                    for assetIndex = 1:obj.nAssets
                        obj.localSimulations{assetIndex}.ekf.declaredCommonProcessNoiseGroup_ = ...
                            commonProcessNoiseGroup;
                    end
                end
            end
    ...
    methods (Static, Access = private)
        ...
        function requireCorrelationNetworkConfiguration_(cfg, settings, isPerAssetLeaf)
            ...
            % Section 3.3: commonSourceTreatment.towerClockProduct='rejected' (required above,
            % unconditionally) is a FALSE claim whenever this network genuinely asserts P_ij=0
            % between two tracked leaves that both observe a shared tower's broadcast clock
            % correction product. models.clocks.TowerClockCorrectionProvider.productNoise_'s
            % correction residual is a DETERMINISTIC function of (towerIndex,productEpoch) only
            % -- not merely cached, but mathematically identical for every consumer of that
            % (tower,epoch) pair regardless of process/session/asset -- so two tracked leaves
            % observing the same tower in the same product window share an identical, real,
            % non-averaging common-mode error the correlation network's Q_ij-only propagation
            % rule (DistributedCovarianceNetwork.advanceEpoch) does not carry (the missing term
            % is measurement-space K_i*R_ij*K_j', not process noise -- see revgnss.
            % DistributedLinkProtocolContract.CommonSourceNames's own comment). Only
            % 'perfectCorrection' (exact truth clock, zero residual) is genuinely free of this;
            % every other towerClockMode has SOME uncorrected or deterministic-shared residual.
            % Scoped to policy='exactPairwiseCrossCovariance' specifically (not every independent-
            % fleet run): this is the one component that actually asserts P_ij=0 and audits
            % Cauchy-Schwarz on that assertion, so it is the one place the false claim has a
            % concrete, checkable consequence -- a Stage-1/2 fleet with no correlation network
            % makes no cross-covariance claim linkUpdate.commonSourceTreatment could falsify.
            if nAssets > 1 && ~strcmp(models.clocks.TowerClockCorrectionProvider.towerClockMode(cfg), ...
                    'perfectCorrection')
                error('IndependentFleetCoordinator:towerClockProductReachableButRejected', ...
                    ['estimator.towerClockMode~=''perfectCorrection'' with nSpaceAssets>1 and an ' ...
                    'enabled correlation network makes commonSourceTreatment.towerClockProduct=' ...
                    '''rejected'' a false claim for any pair of tracked leaves sharing a visible ' ...
                    'tower: TowerClockCorrectionProvider''s correction residual is a deterministic ' ...
                    'function of (tower,productEpoch), identical for every real consumer of that ' ...
                    'pair, not merely cached. No treatment exists yet for this combination ' ...
                    '(Section 3.3 scope note); it is refused outright rather than silently ' ...
                    'mislabeled.']);
            end
            revgnss.DistributedLinkProtocolContract.requireCommonSourceTreatmentDeclared( ...
                settings.linkUpdate.commonSourceTreatment);
            ...
            % 5. Common process noise (Section 3.3): now live -- initialize() declares the group
            % on the network AND hands the identical (isequal) value to every leaf's own buildQ_
            % (see filter.ReverseGNSSEKF.declaredCommonProcessNoiseGroup_), closing the diagonal gap
            % Stage 3.1/3.2 deferred here. A declared group with a non-positive magnitude is a
            % pointless/inconsistent declaration (it would silently no-op both consumers rather
            % than fail loudly), so it is refused rather than accepted as a no-op.
            if strcmp(treatment,'declaredCommonAccelerationGroup') && ...
                    ~(isnumeric(net.commonProcessNoise.sigma_mps2) && ...
                    isscalar(net.commonProcessNoise.sigma_mps2) && net.commonProcessNoise.sigma_mps2 > 0)
                error('IndependentFleetCoordinator:commonProcessNoiseGroupMagnitudeRequired', ...
                    ['commonProcessNoiseTreatment=''declaredCommonAccelerationGroup'' requires ' ...
                    'correlationNetwork.commonProcessNoise.sigma_mps2 > 0: a declared group with a ' ...
                    'non-positive magnitude is a pointless declaration, refused rather than silently ' ...
                    'treated as a no-op.']);
            end
