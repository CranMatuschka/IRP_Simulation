classdef FederatedSwarmSummary
    ...
    methods (Static)
        ...
        function print(out)
            ...
            fprintf('  %-6s %12s %12s %8s %12s %14s\n', 'asset', 'absErr[m]', 'absSig[m]', 'err/sig', 'clkErr[m]', 'relPosErr[m]');
            for i = 1:numel(out.perAsset)
                p = out.perAsset(i);
                refTag = ''; if p.asset == out.refAsset; refTag = '  (ref)'; end
                fprintf('  %-6d %12.3f %12.3f %8.2f %12.4f %14.4f%s\n', ...
                    p.asset, p.absErr_m, p.absSigma_m, p.absRatio, p.clkErr_m, p.relPosErr_m, refTag);
            end
            f = out.formation;
