    ...
    methods (Static)
        function validateConfig(cfg)
            ...
            if revgnss.ISLMeasurementBuilder.getBool_(cfg, {'measurements','isl','carrier','useInEKF'}, false)
                ...
                % Over-tight carrier weighting is the measured failure mode (see the
                % masterConfig table): the row cannot observe the (b_rx, -B) direction, so
                % weighting it far below the 0.3 m code datum lets it dominate a direction
                % it does not constrain, and the error grows with arc length. Warn rather
                % than error -- a user may legitimately probe this -- but never let it pass
                % silently, because the symptom (slow divergence) looks like physics.
                sigC = revgnss.ISLMeasurementBuilder.getNum_(cfg, ...
                    {'measurements','isl','carrier','sigma_m'}, 0.20);
                if sigC < 0.05
                    warning('ISLMeasurementBuilder:carrierSigmaOptimistic', ...
                        ['ISL carrier sigma_m = %.4g m is below the 0.05 m level at which ' ...
                         'the filter was measured to stay consistent. At 0.002 m a 3600 s ' ...
                         'run degraded position 23x and drove the ambiguity error to 2.1x ' ...
                         'its own sigma. sigma_m is the TOTAL error budget, not thermal ' ...
                         'noise alone.'], sigC);
                end
