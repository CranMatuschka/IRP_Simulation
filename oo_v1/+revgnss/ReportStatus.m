classdef ReportStatus
    ...
    methods (Static)
        ...
        function s = current()
            s.stage      = '72';
            s.stageTitle = 'Clock Product Consistency and Navigation Recovery';
            ...
            s.validationArtifactFresh = (vsStageNum >= 72) && strcmp(vsSHA, runtimeSHA);
            if ~s.validationArtifactFresh
                s.validationWarnings{end+1} = ...
                    'No fresh local validation summary for this commit. Run: setenv(''OO_V1_VALIDATE_REPORT'',''true''); setenv(''OO_V1_VALIDATION_STAGE'',''72''); run_oo_reverse_gnss_report';
            end
