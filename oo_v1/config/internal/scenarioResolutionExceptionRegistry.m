...
%   never a value the resolution actually overrode.
    ...
    % ionosphere-free fix, extended 2026-08-08 with the config/ladder/freq axis, and
    % again 2026-08-22 with the freq009-013 band rungs and the four best/ stack rungs.
    % THE GAP THIS CLOSES: freq009-013 and the best rungs were created AFTER the
    % 2026-08-08 extension and nobody came back to register them, so the invariance
    % test has been failing on them ever since. Registering a file here is a TEST-ONLY
    % statement -- this registry has exactly one consumer, test_scenario_override_
    % invariance, and is read by nothing in the resolution path -- so an entry can
    % never change a resolved config, a metric or a golden. It only records that a
    % known shape normalisation is expected on that file.
    % STILL NOT CLOSED: carr010 and carr011 set no mask of their own. They _extends
    % freq008, which IS registered, and inherit the leaf along with its violation --
    % but the lookup is keyed on the LEAF filename, so a parent's exception does not
    % travel to the child that inherits the exempted leaf. Naming them here would
    % work and would be treating the symptom: the next rung to extend freq008 breaks
    % again. The structural fix is for the lookup to follow _extends, which is a
    % design change to this registry and is deliberately NOT made here.
    maskFiles = { ...
        'freq001_L1only.json', ...
        ...
        'freq009_ism915_2450.json', ...
        'freq010_ism2450_5800.json', ...
        'freq011_unii5200_5800.json', ...
        'freq012_ism5800_24125.json', ...
        'freq013_ism24125_61250.json', ...
        'freq014_kaIonoFree.json', ...
        'best001_stackSingleAsset.json', ...
        'best002_stackLowBand.json', ...
        'best101_stackMultiAsset.json', ...
        'best102_stackMultiAssetLowBand.json'};
