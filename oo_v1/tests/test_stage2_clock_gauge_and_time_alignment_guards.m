...
% Section 2.4 itself added neither vocabulary; the four entries present here are Section
% 2.3.1's (coherentTwoWayCodeRange), Section 2.3.2's (firstOrderReciprocalClockTransfer), and
% Section 2.3 item 3's (oneWayCode, oneWayDoppler) own sanctioned widenings, each with its own
% dedicated adapter and stage-acceptance test.
assert(isequal(revgnss.DistributedLinkUpdateAdapter.AllowedObservables, ...
    ...
    'AllowedObservables must carry exactly the observables sanctioned through Section 2.3 item 3.');
assert(isequal(revgnss.DistributedLinkUpdateAdapter.RegisteredAdapterClasses, ...
    ...
    'RegisteredAdapterClasses must carry exactly the four sanctioned adapters.');
