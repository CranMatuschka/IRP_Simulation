...
% FLIPPED 2026-08-09: the default was true, which made the ground oscillator inert (see
% Part C). It is now false, so the five towers actually run the OCXO they are labelled
% with, and the wander they accumulate over the product age is charged to R by
% TowerClockCorrectionProvider.extrapolationWanderVar_.
assert(base.clock.tower.deterministic == false, ...
    'Part A FAILED: the default clock.tower.deterministic must be false (oscillators run)');
...
for k = 1:numel(cGold.towers)
    ...
    assert(cGold.towers(k).clock.deterministic == false, ...
        'Part A FAILED: tower %d is still deterministic; the knob did not reach it', k);
end
...
for spec = {{'RUBIDIUM', false}, {'CESIUM1', false}, {'OCXO', true}}
    ...
    tmpl = revgnss.ConfigFactory.getClockTemplate_(want);
...
scaling = struct('globalBiasFactor',1, ...
