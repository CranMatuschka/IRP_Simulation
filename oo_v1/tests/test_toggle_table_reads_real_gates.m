...
%       saying so in the note is honest, printing it as a STATUS implies it is active
%   T4  the ionosphere note distinguishes CODE from CARRIER. The IF combination is gated
...
% ----------------------------------------------------------------
fprintf('  T1: three-channel atmosphere table, binary per channel ...\n');
rows = CE.atmosphereEffectRows_(cfg);
...
iIono = find(strcmpi(rows(:,1),'Ionosphere'), 1);
...
% ----------------------------------------------------------------
fprintf('  T4: ionosphere note distinguishes code from carrier ...\n');
...
rA = CE.atmosphereEffectRows_(cA); nA = lower(rA{iIono,5});
rB = CE.atmosphereEffectRows_(cB); nB = lower(rB{iIono,5});
assert(~strcmp(nA, nB), ...
...
rC = CE.atmosphereEffectRows_(cC); nC = lower(rC{iIono,5});
...
assert(~isempty(regexp(src, "Carrier phase in EKF", 'once')), ...
    'T5c FAILED: the carrier row was not re-pointed at measurements.carrierMode');
fprintf('    carrier row follows measurements.carrierMode: PASS\n');
