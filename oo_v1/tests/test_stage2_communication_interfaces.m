...
rejectionRecord = struct('observationIdentifier','obs:rejected:s21','ownerAssetIdentifier', ...
    'spacecraft:1','remoteProductIdentifier','estimatorProduct:none','sourceEpoch_s',epoch_s, ...
...
% Section 2.3.1 registers exactly ONE concrete per-observable adapter
% (revgnss.CoherentTwoWayRangeLinkUpdateAdapter, for 'coherentTwoWayCodeRange'); every other
% real observable identifier -- including every entry still in ReservedFutureObservables --
% remains refused, and an unrelated adapter class name remains unregistered.
assert(isequal(revgnss.DistributedLinkUpdateAdapter.RegisteredAdapterClasses, ...
    {'revgnss.CoherentTwoWayRangeLinkUpdateAdapter'}), ...
    'Exactly one concrete per-observable adapter must be registered.');
...
assert(numel(reserved) == 3 && ~any(strcmp('coherentTwoWayCodeRange',reserved)), ...
    'coherentTwoWayCodeRange must no longer be listed as reserved/unimplemented.');
for idx = 1:numel(reserved)
    i_expectError_(@() revgnss.DistributedLinkUpdateAdapter.requireObservableSelectable( ...
        reserved{idx}),'DistributedLinkUpdateAdapter:observableNotSelectable');
end
i_expectError_(@() revgnss.DistributedLinkUpdateAdapter.requireObservableSelectable( ...
