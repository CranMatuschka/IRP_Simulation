...
assert(ismember(s1.attitudeInitConfidenceClass, VALID_CONF), ...
    'T1 FAILED: unknown confidence class %s', s1.attitudeInitConfidenceClass);
assert(all(isfinite(s1.attitudeInitBestEuler_deg)), ...
    'T1 FAILED: best candidate Euler missing');
assert(all(isfinite(s1.attitudeInitSecondEuler_deg)), ...
    'T1 FAILED: second candidate Euler missing');
assert(isfinite(s1.attitudeInitCandidateError_deg), ...
    'T1 FAILED: candidate truth error missing');
assert(isfinite(s1.attitudeInitCandidateImprovementRatio), ...
    'T1 FAILED: improvement ratio missing');
assert(numel(s1.attitudeInitTopResidualCycles) >= 2, ...
    'T1 FAILED: residual surface top candidates missing');
assert(size(s1.attitudeInitTopEuler_deg, 1) == 3, ...
    'T1 FAILED: top Euler candidate matrix has wrong shape');
fprintf('    PASS (confidence=%s, ratio=%.3f, candErr=%.3f deg, imp=%.3f)\n', ...
    s1.attitudeInitConfidenceClass, s1.attitudeInitRatio, ...
    s1.attitudeInitCandidateError_deg, s1.attitudeInitCandidateImprovementRatio);

% ----------------------------------------------------------------
% T2: weak/rejected candidate is not injected into main EKF
% ----------------------------------------------------------------
fprintf('  T2: rejected candidates are diagnostic-only ...\n');
if ~strcmp(s1.attitudeInitConfidenceClass, 'ACCEPTED_ABS_ATTITUDE')
    assert(~s1.attitudeInitAcceptedByEkf, ...
...
if s1.attitudeInitCandidateImprovementRatio >= 1.05 && s1.attitudeInitRatio < 1.20
    assert(ismember(s1.attitudeInitConfidenceClass, ...
        {'WEAK_ATTITUDE_DETECTED','AMBIGUOUS_ATTITUDE'}), ...
        'T3 FAILED: improved but rejected candidate should be weak or ambiguous');
end
assert(~isempty(s1.attitudeInitDecisionReason), ...
    'T3 FAILED: decision reason missing');
fprintf('    PASS (%s)\n', s1.attitudeInitDecisionReason);

% ----------------------------------------------------------------
% T4: shadow mode remains disabled by default
% ----------------------------------------------------------------
fprintf('  T4: shadow mode default is disabled ...\n');
assert(strcmp(s1.attitudeInitShadowMode, 'DISABLED'), ...
